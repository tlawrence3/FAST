...
package FAST::Bio::OntologyIO::Handlers::InterProHandler;
use strict;
use Carp;
...
=head2 new
 ...
 Title   : new
 Usage   : $h = FAST::Bio::OntologyIO::Handlers::InterProHandler->new;
...
sub new {
    ...
    if ( defined($ont) ) {
        ...
        $self->ontology( FAST::Bio::Ontology::Ontology->new( -name => $name ) );
    }
    ...
    $contains_rel = FAST::Bio::Ontology::RelationshipType->get_instance("CONTAINS");
    $found_in_rel = FAST::Bio::Ontology::RelationshipType->get_instance("FOUND_IN");
...
sub ontology_engine {
    ...
    if ( defined $value ) {
        if ( defined $self->{'ontology_engine'} ) {
            ...
            $self->throw(
                ref($value) . " does not implement " . "FAST::Bio::Ontology::OntologyEngineI. Bummer." )
                unless $value->isa("FAST::Bio::Ontology::OntologyEngineI");
...
=head2 ontology
 ...
 Example :
 Returns : A L<FAST::Bio::Ontology::OntologyI> implementing object.
 Args    : On set, a L<FAST::Bio::Ontology::OntologyI> implementing object.
...
sub ontology {
    ...
    if ( defined($ont) ) {
        $self->throw( ref($ont) . " does not implement FAST::Bio::Ontology::OntologyI" . ". Bummer." )
            unless $ont->isa("FAST::Bio::Ontology::OntologyI");
        $self->{'_ontology'} = $ont;
    }
...
=head2 term_factory
 ...
 Example :
 Returns : value of term_factory (a FAST::Bio::Factory::ObjectFactory instance)
 Args    : on set, new value (a FAST::Bio::Factory::ObjectFactory instance
           or undef, optional)
...
sub _create_relationship {
    ...
    my $rel = FAST::Bio::Ontology::Relationship->new( -predicate_term => $rel_type_term );
...
sub start_element {
    ...
    if ( $element->{Name} eq 'interprodb' ) {
        ...
        my $rel = FAST::Bio::Ontology::Relationship->new( -predicate_term => $is_a_rel );
