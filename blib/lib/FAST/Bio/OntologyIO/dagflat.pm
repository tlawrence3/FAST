#
# BioPerl module for FAST::Bio::OntologyIO::dagflat
#
...
=head1 SYNOPSIS
  ...
  use FAST::Bio::OntologyIO;

  # do not use directly -- use via FAST::Bio::OntologyIO
  # e.g., the GO parser is a simple extension of this class
  my $parser = FAST::Bio::OntologyIO->new
        ( -format       => "go",
  ...
  my $IS_A       = FAST::Bio::Ontology::RelationshipType->get_instance( "IS_A" );
  my $PART_OF    = FAST::Bio::Ontology::RelationshipType->get_instance( "PART_OF" );
  my $RELATED_TO = FAST::Bio::Ontology::RelationshipType->get_instance( "RELATED_TO" );
...
package  FAST::Bio::OntologyIO::dagflat;
...
use FAST::Bio::Ontology::OntologyStore;
...
use base qw(FAST::Bio::OntologyIO);


=head2 new

 Title   : new
 Usage   : $parser = FAST::Bio::OntologyIO->new(
                             -format => "go",
 ...
 Returns : A new dagflat parser object, implementing FAST::Bio::OntologyIO.
 Args    : -defs_file  => the name of the file holding the term
           ...
           -ontology_name => the name of the ontology; if not specified the
                          ...
                          to spaces
           -engine     => the FAST::Bio::Ontology::OntologyEngineI object
                          to be reused (will be created otherwise); note
                          that every FAST::Bio::Ontology::OntologyI will
                          qualify as well since that one inherits from the
                          former.

See L<FAST::Bio::OntologyIO>.
...
sub _initialize {
  ...
  $eng = FAST::Bio::Ontology::OBOEngine->new() unless $eng;
  if($eng->isa("FAST::Bio::Ontology::OntologyI")) {
...
=head2 parse
 ...
 Returns : [FAST::Bio::Ontology::OntologyEngineI]
 Args    :
...
sub parse {
    ...
    $self->term_factory(FAST::Bio::Ontology::TermFactory->new(
                                             -type => "FAST::Bio::Ontology::Term"))
        unless $self->term_factory();

    # create the ontology object itself
    my $ont = FAST::Bio::Ontology::Ontology->new(-name => $self->ontology_name(),
...
=head2 next_ontology
 ...
 Function: Get the next available ontology from the parser. This is the
           method prescribed by FAST::Bio::OntologyIO.
 Example :
 Returns : An object implementing FAST::Bio::Ontology::OntologyI, and undef if
...
sub next_ontology {
  ...
  if(exists($self->{'_ontologies'})){
    ...
    if($ont){
      my $store = FAST::Bio::Ontology::OntologyStore->new();
...
sub defs_file {
  ...
  if ( @_ ) {
    ...
    $self->{ "_defs_file_name" } = $f;
        ...
        if(defined($f)) {
      $self->_defs_io( FAST::Bio::Root::IO->new( -input => $f ) );
    }
  }
...
sub defs_url {
  ...
  if(defined($val)){
    $self->{'_defs_url'} = $val;
        ...
        $self->_defs_io->close() if $self->_defs_io();
    $self->_defs_io( FAST::Bio::Root::IO->new( -url => $val ) );
  }
...
=head2 _defs_io
 ...
 Function: Get/set the FAST::Bio::Root::IO instance representing the
           definition file, if provided (see defs_file()).
 Example :
 Returns : value of _defs_io (a FAST::Bio::Root::IO object)
 Args    : on set, new value (a FAST::Bio::Root::IO object or undef, optional)
...
sub _add_ontology {
    ...
    foreach my $ont (@_) {
        $self->throw(ref($ont)." does not implement FAST::Bio::Ontology::OntologyI")
            unless ref($ont) && $ont->isa("FAST::Bio::Ontology::OntologyI");
...
# convert simple strings to FAST::Bio::Annotation::DBLinks
sub _to_annotation {
    my ($self , $links) = @_;
    return unless $links;
    my @dbxrefs;
