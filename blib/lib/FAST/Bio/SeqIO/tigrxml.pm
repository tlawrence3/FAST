#
# BioPerl module for FAST::Bio::SeqIO::tigrxml
#
...
=head1 SYNOPSIS
  ...
  use FAST::Bio::SeqIO;
  my $in = FAST::Bio::SeqIO->new(-format => 'tigrcoordset',
                          -file   => 'file.xml');
...
package FAST::Bio::SeqIO::tigrxml;
...
use base qw(FAST::Bio::SeqIO XML::SAX::Base);
...
sub start_element {
	...
	my $f = FAST::Bio::SeqFeature::Generic->new
	    (-seq_id      => $seqid,
	     -start       => $s,
	     -end         => $e,
	     -strand      => $strand,
...
sub end_element {
	...
	if( $curseq->can('annotation') ) {
		...
		('reference',FAST::Bio::Annotation::Reference->new
	...
	while( defined $exon && $exon->start < $cdsexon->start ) {
	    ...
	    my $utr = FAST::Bio::SeqFeature::Generic->new
		(-seq_id      => $exon->seq_id,
		 -strand      => $exon->strand,
...
sub characters {
	...
	if( defined $curseq ) { 
		...
		$curseq->species(FAST::Bio::Species->new(
						   -classification => 
						   [$species,$genus],
						   -sub_species => $species));
	    } elsif( $name eq 'LINEAGE' ) {
