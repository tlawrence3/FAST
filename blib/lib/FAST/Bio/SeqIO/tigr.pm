# BioPerl module for FAST::Bio::SeqIO::tigr
#
...
This object can transform FAST::Bio::Seq objects to and from efa flat
file databases.
...
# Let the code begin...
package FAST::Bio::SeqIO::tigr;
use strict;

use FAST::Bio::Seq::RichSeq;
use FAST::Bio::Species;
use FAST::Bio::Annotation::Comment;
use FAST::Bio::SeqFeature::Generic;
...
sub next_seq()
{
    ...
    if($start < 0) {
        throw FAST::Bio::Root::OutOfRange("the sequence start is $start < 0");
    } elsif($end > length($self->{_assembly}->{seq})) {
        throw FAST::Bio::Root::OutOfRange("the sequence end is $end < " . length($self->{_assembly}->{seq}));
    } elsif($start >= $end) {
        throw FAST::Bio::Root::OutOfRange("the sequence start is after end $start >= $end");
    }
	...
	foreach my $feat ($seq->get_SeqFeatures()) {
		...
		try {
			# Get the subsq
			my $cds = FAST::Bio::PrimarySeq->new(
				-strand => 1,
...
sub _process_tigr
{
	...
	$line = $self->_readline();
	if($line !~ /<TIGR>/o) {
		$self->throw("FAST::Bio::SeqIO::tigr::_process_tigr called but no ",
...
sub _process_assembly
{
	...
	$line = $self->_readline();
	if($line !~ /<ASSEMBLY([^>]*)>/o) {
		$self->throw("FAST::Bio::SeqIO::tigr::_process_assembly called ",
...
sub _process_assembly_seq()
{
	...
	$line = $self->_readline();
	if($line !~ /<ASSEMBLY_SEQUENCE>/o) {
		$self->throw("FAST::Bio::SeqIO::tigr::_process_assembly_seq called ".
...
sub _process_coordset($)
{
	...
	if($line =~ /<COORDSET>/o) {
		...
		$self->throw("FAST::Bio::SeqIO::tigr::_process_coordset() called ",
...
sub _process_header
{
	...
	if($line !~ /<HEADER>/o) {
		$self->throw("FAST::Bio::SeqIO::tigr::_process_header called ",
	...
	if($line =~ /<ORGANISM>\s*(.+)\s*<\/ORGANISM>/o) {
		...
		$self->{_assembly}->{species} = FAST::Bio::Species->new();
...
sub _process_gene_list
{
	...
	$line = $self->_readline();
	if($line !~ /<GENE_LIST>/o) {
		$self->throw("FAST::Bio::SeqIO::tigr::_process_gene_list called ",
...
sub _process_protein_coding
{
	...
	if($line !~ /<PROTEIN_CODING>/o) {
		$self->throw("FAST::Bio::SeqIO::tigr::_process_protein_coding called"
		             . "but no <GENE_LIST> in the stream");
	}
...
sub _process_rna_genes
{
	...
	if($line =~ /<RNA_GENES>/o) {
		...
		$self->throw("FAST::Bio::SeqIO::tigr::_process_rna_genes called ",
...
sub _process_misc_info
{
	...
	if($line =~ /<MISC_INFO>/o) {
		...
		$self->throw("FAST::Bio::SeqIO::tigr::_process_misc_info called ",
...
sub _process_repeat_list
{
	...
	if($line =~ /<REPEAT_LIST>/o) {
		...
		$self->throw("FAST::Bio::SeqIO::tigr::_process_repeat_list called ",
		             "but no <MISC_INFO> in the stream");
	}
}
...
sub _process_tu
{
	...
	try {
		my $tu = FAST::Bio::Seq::RichSeq->new(-strand => 1);
		$tu->species( $self->{_assembly}->{species} );

		# Add the source tag, so we can add the GO annotations to it
		$tu->add_SeqFeature(FAST::Bio::SeqFeature::Generic->new(-source_tag => 'TIGR', -primary_tag => 'source'));
	...
	} catch FAST::Bio::Root::OutOfRange with {
		my $E = shift;
...
#		$self->throw("Invalid Pub_Locus");
	...
	while($line =~ /<COMMENT>([^<]+)<\/COMMENT>/o) {
		my $comment = FAST::Bio::Annotation::Comment->new(
			-text => $1
		);
...
sub _build_location
{
	...
	if( $start < 1 ) {
		throw FAST::Bio::Root::OutOfRange(
	...
	return FAST::Bio::Location::Simple->new( -start => $start, -end => $end, -strand => 1 );
}
...
sub _process_model
{
	...
	my $line;
	my( $source ) = grep { $_->primary_tag() eq 'source' } $tu->get_SeqFeatures();
	my $model = FAST::Bio::SeqFeature::Generic->new(
		-source_tag  => 'TIGR',
		-primary_tag => 'MODEL',
	);
...
#		$self->throw("Invalid Pub_Locus: $line");
	...
	my $exon = FAST::Bio::SeqFeature::Generic->new(
		-source_tag  => 'TIGR',
		-primary_tag => 'EXON',
		-location => FAST::Bio::Location::Split->new(),
		-tags => [ locus => $tu->accession_number() ],
	);
	$exon->add_tag_value( model => $model->get_tag_values('feat_name') );

	my $cds  = FAST::Bio::SeqFeature::Generic->new(
		-source_tag  => 'TIGR',
		-primary_tag => 'CDS',
		-location => FAST::Bio::Location::Split->new(),
		-tags => [ locus => $tu->accession_number() ],
...
sub _process_cdna_support
{
	...
	if($line !~ /<CDNA_SUPPORT>/o) {
		$self->throw("FAST::Bio::SeqIO::tigr::_process_cdna_support called ",
...
sub _process_exon
{
	...
	if($line !~ /<EXON>/o) {
		$self->throw("FAST::Bio::SeqIO::tigr::_process_exon called ",
	...
	$self->throw("Reached End of FAST::Bio::SeqIO::tigr::_process_exon");
}
...
sub _process_cds
{
	...
	if($line !~ /<CDS>/o) {
		$self->throw("FAST::Bio::SeqIO::tigr::_process_cda_support called ",
	...
	$self->throw("Reached onf of FAST::Bio::SeqIO::tigr::_process_cds");
}
...
sub _process_utrs
{
	...
	if($line !~ /<UTRS/o) {
		$self->throw("FAST::Bio::SeqIO::tigr::_process_utrs called but no ",
	...
	$self->throw("Reached end of FAST::Bio::SeqIO::tigr::_process_utrs");
}
...
sub _process_left_utr
{
	...
	if($line !~ /<LEFT_UTR>/o) {
		$self->throw("FAST::Bio::SeqIO::tigr::_process_left_utr called but ",
	...
	if($line =~ /<COORDSET>/o) {
		...
		push(@$utrs, FAST::Bio::SeqFeature::Generic->new(
		        -source_tag  => 'TIGR',
	...
	$self->throw("Reached end of FAST::Bio::SeqIO::tigr::_process_left_utr");
}
...
sub _process_right_utr
{
	...
	if($line !~ /<RIGHT_UTR>/o) {
		$self->throw("FAST::Bio::SeqIO::tigr::_process_right_utr called but ",
	...
	$self->throw("Reached end of FAST::Bio::SeqIO::tigr::_process_right_utr");
}
...
sub _process_ext_utr
{
	...
	if($line !~ /<EXTENDED_UTR>/o) {
		$self->throw("FAST::Bio::SeqIO::tigr::_process_ext_utr called but ",
	...
	$self->throw("Reached end of FAST::Bio::SeqIO::tigr::_process_ext_utr");
}
