# BioPerl module for FAST::Bio::SeqIO::fasta
#
...
This object can transform FAST::Bio::Seq objects to and from fasta flat
file databases.
...
=head2 next_seq
 ...
 Returns : FAST::Bio::Seq object, or nothing if no more available
 Args    : NONE
...
sub next_seq {
    ...
    ## THE FOLLOWING COMMENTED BY DHA FOR BETTER PROCESSING OF DATA FROM STDIN
    #DHA: $self->throw("The sequence does not appear to be FASTA format ".
    #DHA:    "(lacks a descriptor line '>')") if $. == 1 && $entry !~ /^>/;
...
=head2 write_seq
 ...
 Args    : Array of 1 or more FAST::Bio::PrimarySeqI objects
...
sub write_seq {
   ...
   my $width = $self->width;
   foreach my $seq (@seq) {
		$self->throw("Did not provide a valid FAST::Bio::PrimarySeqI object") 
		  unless defined $seq && ref($seq) && $seq->isa('FAST::Bio::PrimarySeqI');
		...
		if( $seq->isa('FAST::Bio::Seq::LargeSeqI') ) {
		  $self->_print(">$top\n");
...
=head2 preferred_id_type
 ...
 Function: Get/Set the preferred type of identifier to use in the ">ID" position
           for FASTA output.
 Returns : string, one of values defined in @FAST::Bio::SeqIO::fasta::SEQ_ID_TYPES.
           Default = $FAST::Bio::SeqIO::fasta::DEFAULT_SEQ_ID_TYPE ('display').
 Args    : string when setting. This must be one of values defined in 
           @FAST::Bio::SeqIO::fasta::SEQ_ID_TYPES. Allowable values:
