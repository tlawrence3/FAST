#
# BioPerl module for FAST::Bio::Taxon
#
...
=head1 SYNOPSIS
  ...
  # Typically you will get a Taxon from a FAST::Bio::DB::Taxonomy object
  # but here is how you initialize one
  my $taxon = FAST::Bio::Taxon->new(-name      => $name,
                             -id        => $id,
                             -rank      => $rank,
                             -division  => $div);

  # Get one from a database
  my $dbh = FAST::Bio::DB::Taxonomy->new(-source   => 'flatfile',
                                  -directory=> '/tmp',
  ...
  my $list_dbh = FAST::Bio::DB::Taxonomy->new(-source => 'list', -names => \@h_lineage,
                                                          -ranks => \@ranks);
  ...
  my $entrez_dbh = FAST::Bio::DB::Taxonomy->new(-source => 'entrez');
  $human->db_handle($entrez_dbh);
  @names = $human->common_names; # @names contains woman, human, man

  # Since FAST::Bio::Taxon implements FAST::Bio::Tree::NodeI, we have access to those
  ...
  # yourself, you won't get an answer because unlike for ancestor(), FAST::Bio::Taxon
  # does not ask the database for the answer. You can ask the database yourself
  # using the same method:
  ($human) = $homo->db_handle->each_Descendent($homo);

  # We can also take advantage of FAST::Bio::Tree::Tree* methods:
  # a) some methods are available with just an empty tree object
  use FAST::Bio::Tree::Tree;
  my $tree_functions = FAST::Bio::Tree::Tree->new();
  ...
  my $tree = FAST::Bio::Tree::Tree->new(-node => $human);
  ...
  my $mouse_tree = FAST::Bio::Tree::Tree->new(-node => $entrez_mouse);
...
object called FAST::Bio::Species. This new implementation allows
...
Jason Stajich,    jason-at-bioperl-dot-org (original FAST::Bio::Taxonomy::Node)
...
package FAST::Bio::Taxon;
use strict;
use Scalar::Util qw(blessed);

use FAST::Bio::DB::Taxonomy;

use base qw(FAST::Bio::Tree::Node FAST::Bio::IdentifiableI);

=head2 new

 Title   : new
 Usage   : my $obj = FAST::Bio::Taxonomy::Node->new();
 Function: Builds a new FAST::Bio::Taxonomy::Node object 
 Returns : an instance of FAST::Bio::Taxonomy::Node
 Args    : -dbh               => a reference to a FAST::Bio::DB::Taxonomy object
                                 [no default]
...
sub new {
    ...
    # some things want to freeze/thaw FAST::Bio::Species objects, but
...
=head1 FAST::Bio::IdentifiableI interface 

Also see L<FAST::Bio::IdentifiableI>
...
=head1 FAST::Bio::Taxonomy::Node implementation

=head2 db_handle

 Title   : db_handle
 Usage   : $taxon->db_handle($newval)
 Function: Get/Set FAST::Bio::DB::Taxonomy Handle
 Returns : value of db_handle (a scalar) (FAST::Bio::DB::Taxonomy object)
 Args    : on set, new value (a scalar, optional) FAST::Bio::DB::Taxonomy object

Also see L<FAST::Bio::DB::Taxonomy>
...
sub db_handle {
    ...
    if (@_) {
        ...
        if (! ref($db) || ! $db->isa('FAST::Bio::DB::Taxonomy')) {
            $self->throw("Must provide a valid FAST::Bio::DB::Taxonomy object to db_handle()");
        }
        ...
        # NB: The FAST::Bio::DB::Taxonomy modules access this data member directly
...
=head2 ancestor
 ...
 Function: Retrieve the ancestor taxon. Normally the database is asked what the
           ...
           If you manually set the ancestor (or you make a FAST::Bio::Tree::Tree with
...
=head2 each_Descendent
 ...
 Function: Get all the descendents for this Taxon (but not their descendents,
           ...
           FAST::Bio::Tree::Tree with this object as an argument to new().
...
# implemented by FAST::Bio::Tree::Node
...
sub _get_similar_taxon_from_db {
    ...
    if (!blessed($db) || !$db->isa('FAST::Bio::DB::Taxonomy')) {
        $self->throw("DB handle is not a FAST::Bio::DB::Taxonomy: got $db in node ".$self->node_name)
    }
...
sub _merge_taxa {
    my ($self, $taxon) = @_;
    $self->throw("Must supply a FAST::Bio::Taxon object") unless ref($taxon) && $taxon->isa('FAST::Bio::Taxon');
...
sub remove_Descendent {
    # need to override this method from FAST::Bio::Tree::Node since it casually
