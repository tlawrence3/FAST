#
# BioPerl module for FAST::Bio::SeqFeature::Gene::Transcript
#
...
package FAST::Bio::SeqFeature::Gene::Transcript;
use strict;


use FAST::Bio::PrimarySeq;

use base qw(FAST::Bio::SeqFeature::Generic FAST::Bio::SeqFeature::Gene::TranscriptI);
...
=head2 promoters
 ...
 Returns : An array of FAST::Bio::SeqFeatureI implementing objects representing the
...
sub promoters {
    my ($self) = @_;
    return $self->get_feature_type('FAST::Bio::SeqFeature::Gene::Promoter');
}
...
sub add_promoter {
    my ($self, $fea) = @_;
    $self->_add($fea,'FAST::Bio::SeqFeature::Gene::Promoter');
}
...
sub flush_promoters {
    my ($self) = @_;
    return $self->_flush('FAST::Bio::SeqFeature::Gene::Promoter');
}
...
sub exons {
    my ($self, $type) = @_;
    return $self->get_unordered_feature_type('FAST::Bio::SeqFeature::Gene::ExonI', 
					     $type);
}
...
sub exons_ordered { 
    my ($self,$type) = @_;
    return $self->get_feature_type('FAST::Bio::SeqFeature::Gene::ExonI', $type);
}
...
=head2 add_exon
 ...
 Returns : 
 Args    : A FAST::Bio::SeqFeature::Gene::ExonI implementing object.
...
sub add_exon {
    my ($self, $fea, $type) = @_;
    if(! $fea->isa('FAST::Bio::SeqFeature::Gene::ExonI') ) {
	$self->throw("$fea does not implement FAST::Bio::SeqFeature::Gene::ExonI");
    }
    $self->_add($fea,'FAST::Bio::SeqFeature::Gene::Exon', $type);
}
...
sub flush_exons {
    my ($self, $type) = @_;
    return $self->_flush('FAST::Bio::SeqFeature::Gene::Exon',$type);
}
...
=head2 introns
 ...
 Returns : An array of FAST::Bio::SeqFeature::Gene::Intron objects representing
           the intron regions.
 Args    : 
...
sub introns {
    ...
    while ((my $exonA = shift (@exons)) &&(my $exonB = shift(@exons))){
       my $intron = FAST::Bio::SeqFeature::Gene::Intron->new(-primary=>'intron');
...
=head2 poly_A_site
 ...
 Returns : A FAST::Bio::SeqFeatureI implementing object representing the
           poly-adenylation region.
 Args    : A FAST::Bio::SeqFeatureI implementing object on set, or FALSE to flush
...
sub poly_A_site {
    my ($self, $fea) = @_;
    if ($fea) {
	$self->_add($fea,'FAST::Bio::SeqFeature::Gene::Poly_A_site');
    }
    return ($self->get_feature_type('FAST::Bio::SeqFeature::Gene::Poly_A_site'))[0];
}
...
=head2 utrs
 ...
 Returns : An array of FAST::Bio::SeqFeature::Gene::UTR objects
...
sub utrs {
    my ($self, $type) = @_;
    return $self->get_feature_type('FAST::Bio::SeqFeature::Gene::UTR',$type);
...
=head2 add_utr
 ...
 Returns : 
 Args    : A FAST::Bio::SeqFeature::Gene::UTR implementing object.
...
sub add_utr {
    my ($self, $fea, $type) = @_;
    $self->_add($fea,'FAST::Bio::SeqFeature::Gene::UTR',$type);
}
...
sub flush_utrs {
    my ($self, $type) = @_;
    return $self->_flush('FAST::Bio::SeqFeature::Gene::UTR',$type);
}
...
sub cds {
    ...
    return unless $cds;
    return FAST::Bio::PrimarySeq->new('-id' => $self->seq_id(),
				'-seq' => $cds,
				'-alphabet' => "dna");
}
...
sub mrna {
    ...
    $seq = $self->cds();
    if(! $seq) {
	$seq = FAST::Bio::PrimarySeq->new('-id' => $self->seq_id(),
				    '-alphabet' => "rna",
				    '-seq' => "");
    }
...
sub _add {
    ...
    require FAST::Bio::SeqFeature::Gene::Promoter;
    require FAST::Bio::SeqFeature::Gene::UTR;
    require FAST::Bio::SeqFeature::Gene::Exon;
    require FAST::Bio::SeqFeature::Gene::Intron;
    require FAST::Bio::SeqFeature::Gene::Poly_A_site;

    if(! $fea->isa('FAST::Bio::SeqFeatureI') ) {
	$self->throw("$fea does not implement FAST::Bio::SeqFeatureI");
    }
    if(! $fea->isa($type) || $pri) {
