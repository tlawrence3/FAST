#
# BioPerl module for FAST::Bio::DB::InMemoryCache
#
...
FAST::Bio::DB::RandomAccessI in memory to a hard limit of sequences.
...
package FAST::Bio::DB::InMemoryCache;
...
use base qw(FAST::Bio::Root::Root FAST::Bio::DB::SeqI);

sub new {
    my ($class,@args) = @_;

    my $self = FAST::Bio::Root::Root->new();
    bless $self,$class;
    ...
    if( !defined $seqdb || !ref $seqdb ||
		  !$seqdb->isa('FAST::Bio::DB::RandomAccessI') ) {
...
sub _load_Seq {
  ...
  if( $self->agr() ) {
      ...
      my $newseq = FAST::Bio::Seq->new( -display_id => $seq->display_id,
