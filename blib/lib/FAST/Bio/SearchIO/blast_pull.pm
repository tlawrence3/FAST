#
# BioPerl module for FAST::Bio::SearchIO::blast_pull
#
...
package FAST::Bio::SearchIO::blast_pull;

use strict;
use FAST::Bio::Search::Result::BlastPullResult;
...
sub _discover_next_result {
    ...
    if ($type eq 'BLASTN' || $type eq 'BLASTP') {
        unless ($self->_sequential) {
            ...
            $self->_fields->{next_result} = FAST::Bio::Search::Result::BlastPullResult->new(-chunk => [($self->chunk, $start, $end)],
                                                                                     -parent => $self);
            ...
            $chunk || return;
            $self->_fields->{next_result} = FAST::Bio::Search::Result::BlastPullResult->new(-chunk => [$chunk],
                                                                                   -parent => $self);
        }
    }
    else {
