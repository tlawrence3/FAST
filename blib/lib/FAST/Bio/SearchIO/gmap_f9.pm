...
#
# BioPerl module for FAST::Bio::SearchIO::gmap_f9
#
...
=head1 SYNOPSIS
   ...
   # FAST::Bio::SearchIO system.
    ...
    my $searchio = FAST::Bio::SearchIO->new(-format => 'gmap',
...
suitable for building FAST::Bio::Search objects from a GMAP "compressed"
report (from gmap run with -Z flag) Read the L<FAST::Bio::SearchIO> for more
...
package FAST::Bio::SearchIO::gmap_f9;
use strict;
use warnings;

use FAST::Bio::Search::Hit::GenericHit;
use FAST::Bio::Search::HSP::GenericHSP;
...
sub next_result {
  ...
  while ( $_ = $self->_readline ) {
	  ...
	  if ($result) {	#    and done if there's one in progress
	      ...
	      $result = FAST::Bio::Search::Result::GenericResult->new();
	      ...
	      $hit ||= FAST::Bio::Search::Hit::GenericHit->new( -name => 
							  "NONE_SPECIFIED");
	  }
      }
...
sub _hsp_from_info {
    ...
    $hsp = FAST::Bio::Search::HSP::GenericHSP->new( %$a );
