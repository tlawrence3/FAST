#
# BioPerl module for FAST::Bio::SearchIO::hmmer_pull
#
...
=head1 SYNOPSIS
    ...
    while (my $result = $in->next_result) {
        # this is a FAST::Bio::Search::Result::HmmpfamResult object
...
package FAST::Bio::SearchIO::hmmer_pull;

use strict;


use base qw(FAST::Bio::SearchIO FAST::Bio::PullParserI);

=head2 new

 Title   : new
 Usage   : my $obj = FAST::Bio::SearchIO::hmmer_pull->new();
 Function: Builds a new FAST::Bio::SearchIO::hmmer_pull object 
 Returns : FAST::Bio::SearchIO::hmmer_pull
 Args    : -fh/-file => HMMER output filename
           ...
           -piped_behaviour defines what the parser should do if the input is
            ...
            FAST::Bio::PullParserI::chunk for details. Default is 'sequential_read'.
...
sub _discover_next_result {
    ...
    if ($type eq 'HMMPFAM') {
        use FAST::Bio::Search::Result::HmmpfamResult;
        ...
        unless ($self->_sequential) {
            ...
            return if $start == $end;
            $self->_fields->{next_result} = FAST::Bio::Search::Result::HmmpfamResult->new(-chunk => [($self->chunk, $start, $end)],
                                                                                   -parent => $self);
            ...
            $chunk || return;
            $self->_fields->{next_result} = FAST::Bio::Search::Result::HmmpfamResult->new(-chunk => [$chunk],
                                                                                   -parent => $self);
        }
    }
    elsif ($type eq 'HMMSEARCH') {
