#
# BioPerl module for FAST::Bio::AlignIO::fasta
#
...
This object can transform L<FAST::Bio::SimpleAlign> objects to and from
...
FastA (FastX, FastN, FastP, tFastA, etc) use the FAST::Bio::SearchIO module.
...
package FAST::Bio::AlignIO::fasta;
use strict;
...
sub next_aln {
	...
	while (defined ($entry = $self->_readline) ) {
	    ...
	    # this is already done in FAST::Bio::PrimarySeq
	    $seqchar .= $entry;
	}
...
sub write_aln {
    ...
    foreach my $aln (@aln) {
	if( ! $aln || ! $aln->isa('FAST::Bio::Align::AlignI')  ) { 
	    $self->warn("Must provide a FAST::Bio::Align::AlignI object when calling write_aln");
	    next;
	}
...
sub _get_len {
	my ($self,$seq) = @_;
	my $chars = $FAST::Bio::LocatableSeq::RESIDUE_SYMBOLS;
	$seq =~ s{[^$chars]+}{}gi;
