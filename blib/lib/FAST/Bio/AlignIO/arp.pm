#
# BioPerl module for FAST::Bio::AlignIO::arp
#
...
Do not use this module directly.  Use it via the L<FAST::Bio::AlignIO> 
class.

=head1 DESCRIPTION

This object can create L<FAST::Bio::SimpleAlign> objects from
...
in a general mapping of where data can be found:
    ...
    Tag        FAST::Bio::Annotation   TagName                    FAST::Bio::Annotation
               Class                                        Parameters
    ----------------------------------------------------------------------
...
FAST::Bio::Annotation::TagTree object in the annotation collection and is accessible
via:
...
See L<FAST::Bio::Align::AlignI>

=cut

sub next_aln {
    my $self = shift;
    my $aln = FAST::Bio::SimpleAlign->new(-source => 'arp');
    ...
    while (defined ($data = $self->_readline) ) {
        ...
        if ($data =~ m{\[{1,2}(\w+)\]{1,2}}xms) {
            ...
            if ($self->{state}->{current_block} eq 'Samples') {
                ...
                # route using a FAST::Bio::Annotation::TagTree. YMMV - cjfields 10-15-08
...
sub _process_sequence {
    ...
    my $ls = FAST::Bio::LocatableSeq->new('-seq'        => $seq,
                                    '-start'      => 1,
                                    '-display_id' => $id,
...
sub _process_annotation {
    ...
    my $factory = FAST::Bio::Annotation::AnnotationFactory->new(-type => 'FAST::Bio::Annotation::SimpleValue');
    for my $anntype (qw(SampleAnnotation Annotation)) {
        for my $key (keys %{ $self->{state}->{$anntype} }) {
            if ($key eq 'Title') {
                ...
                $factory->type('FAST::Bio::Annotation::TagTree');
                $coll->add_Annotation($key, $factory->create_object(
                    -value => [$key => $self->{state}->{$anntype}->{$key}]));
                $factory->type('FAST::Bio::Annotation::SimpleValue');
