...
package FAST::Bio::AlignIO::Handler::GenericAlignHandler;
...
sub build_alignment {
    ...
    if (defined $param->{-seqs}) {
        return FAST::Bio::SimpleAlign->new(%$param, -source => $self->format);
    }
}
...
sub seq_annotation_collection {
    ...
    if ($coll) {
        ...
        $self->{'_params'}->{'-seq_annotation'} = FAST::Bio::Annotation::Collection->new()
    }
...
sub process_seqs {
    ...
    my $class = $data->{-seq_class} || 'FAST::Bio::LocatableSeq';
    ...
    if ( $data->{-consensus_meta} && !UNIVERSAL::isa($data->{-consensus_meta},'FAST::Bio::Seq::Meta')) {
        my $ref = $data->{-consensus_meta};
        if (!exists($self->{'_loaded_modules'}->{'FAST::Bio::Seq::Meta'})) {
            $self->_load_module('FAST::Bio::Seq::Meta');
            $self->{'_loaded_modules'}->{'FAST::Bio::Seq::Meta'}++;
        }
        my $ms = FAST::Bio::Seq::Meta->new();
    ...
    for my $seq (@{$data->{-seqs}}) {
        next if (UNIVERSAL::isa($seq,'FAST::Bio::LocatableI'));
...
# any sequence data for a FAST::Bio::Seq::Meta
sub _generic_metaseq {
    ...
    $self->{'_params'}->{'-seq_class'} = 'FAST::Bio::Seq::Meta';
...
# any sequence data for a FAST::Bio::LocatableSeq
...
sub _generic_reference {
    my ($self, $data) = @_;
    my $ref = FAST::Bio::Annotation::Reference->new(-title => $data->{TITLE},
...
sub _generic_simplevalue {
    my ($self, $data) = @_;
    my $sv = FAST::Bio::Annotation::SimpleValue->new(-value => $data->{DATA},
                                            -tagname  => lc $data->{NAME});
...
sub _generic_comment {
    my ($self, $data) = @_;
    my $comment = FAST::Bio::Annotation::Comment->new(-type => lc $data->{NAME},
                                                -text => $data->{DATA},
                                                -tagname  => lc $data->{NAME});
...
sub _stockholm_target {
    ...
    my $comment;
    # FAST::Bio::Annotation::Target is now a DBLink, but has additional (RangeI) 
    # capabilities (for PDB data)
    my $dblink = FAST::Bio::Annotation::Target->new(
    ...
    if ($data->{ALIGNMENT}) {
        ...
        my $sf = FAST::Bio::SeqFeature::Generic->new(-seq_id => $data->{DISPLAY_ID},
...
#
# BioPerl module for FAST::Bio::AlignIO::Handler::GenericAlignHandler
#
...
FAST::Bio::LocatableSeq or similar (in all other cases it is assumed to be for the
...
=head2 new
 ...
 Args    :  -format    Sequence format to be mapped for handler methods
            -builder   FAST::Bio::Seq::SeqBuilder object (normally defined in
...
=head1 L<FAST::Bio::HandlerBaseI> implementing methods
...
=head2 build_alignment
 ...
 Usage   :  
 Function:  
 Returns :  a FAST::Bio::SimpleAlign
 Args    :
 Throws  :
...
=head2 process_seqs
 ...
 Function:  checks internal sequences to ensure they are converted over
            to the proper FAST::Bio::AlignI-compatible sequence class
