#
# BioPerl module for FAST::Bio::AlignIO::mega
#
...
=head1 SYNOPSIS
    ...
    my $alignio = FAST::Bio::AlignIO->new(-format => 'mega',
...
sub next_aln{
   ...
   my $aln = FAST::Bio::SimpleAlign->new(-source => 'mega');
   ...
   foreach my $seqname ( @order ) {
       my $s = $seqs{$seqname};
       $s =~ s/[$FAST::Bio::LocatableSeq::GAP_SYMBOLS]+//g;
       my $end = length($s);
       my $seq = FAST::Bio::LocatableSeq->new('-alphabet'   => $alphabet,
					'-display_id' => $seqname,
					'-seq'        => $seqs{$seqname},
					'-start'      => 1,
					'-end'        => $end);
