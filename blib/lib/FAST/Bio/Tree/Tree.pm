#
# BioPerl module for FAST::Bio::Tree::Tree
#
...
This implementation of FAST::Bio::Tree::Tree contains FAST::Bio::Tree:::NodeI; mainly linked
via the root node. As NodeI can potentially contain circular references (as
nodes will need to refer to both parent and child nodes), FAST::Bio::Tree::Tree will
...
Example of issue:
  ...
  # root node is passed
  my $root = FAST::Bio::TreeIO->new(-format => 'newick', -file => 'foo.txt')->next_tree
		 ->get_root_node;
...
package FAST::Bio::Tree::Tree;
use strict;

# Object preamble - inherits from FAST::Bio::Root::Root


use base qw(FAST::Bio::Root::Root FAST::Bio::Tree::TreeI FAST::Bio::Tree::TreeFunctionsI);

=head2 new

 Title   : new
 Usage   : my $obj = FAST::Bio::Tree::Tree->new();
 Function: Builds a new FAST::Bio::Tree::Tree object 
 Returns : FAST::Bio::Tree::Tree
 Args    : -root     => L<FAST::Bio::Tree::NodeI> object which is the root
             OR
           -node     => L<FAST::Bio::Tree::NodeI> object from which the root will be
                        determined
...
sub new {
  ...
  if ($node && ! $root) {
    $self->throw("Must supply a FAST::Bio::Tree::NodeI") unless ref($node) && $node->isa('FAST::Bio::Tree::NodeI');
    my @lineage = $self->get_lineage_nodes($node);
    $root = shift(@lineage) || $node;
    
    # to stop us pulling in entire database of a FAST::Bio::Taxon when we later do
    # get_nodes() or similar, specifically set ancestor() for each node
    if ($node->isa('FAST::Bio::Taxon')) {
...
=head2 get_nodes
 ...
 Function: Return list of FAST::Bio::Tree::NodeI objects
...
=head2 set_root_node
 ...
 Returns : FAST::Bio::Tree::NodeI
 Args    : FAST::Bio::Tree::NodeI
...
sub set_root_node{
   ...
   if( @_ ) { 
       my $value = shift;
       if( defined $value && 
       ! $value->isa('FAST::Bio::Tree::NodeI') ) { 
       $self->warn("Trying to set the root node to $value which is not a FAST::Bio::Tree::NodeI");
...
=head2 subtree_length
 ...
 Args    : FAST::Bio::Tree::NodeI object, defaults to the root node
...
=head2 as_text
 ...
 Args    : format type as specified by FAST::Bio::TreeIO
 Note    : This method loads the FAST::Bio::TreeIO::$format module
...
sub as_text {
    ...
    my $iomod = "FAST::Bio::TreeIO::$format";
...
=head2 clone()
 ...
 Function: Safe tree clone that doesn't segfault
           (of Sendu)
 Returns : FAST::Bio::Tree::Tree object
 Args    : [optional] $start_node, FAST::Bio::Tree::Node object
