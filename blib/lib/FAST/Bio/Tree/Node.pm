#
# BioPerl module for FAST::Bio::Tree::Node
#
...
package FAST::Bio::Tree::Node;
use vars qw($CREATIONORDER);
use strict;

use base qw(FAST::Bio::Root::Root FAST::Bio::Tree::NodeI);
...
sub create_node_on_branch{
   ...
   my $node = FAST::Bio::Tree::Node->new(%$annot);
...
sub add_Descendent{
   ...
   if( ! ref($node) ||
       ref($node) =~ /HASH/ ||
       ! $node->isa('FAST::Bio::Tree::NodeI') ) {
       $self->throw("Trying to add a Descendent who is not a FAST::Bio::Tree::NodeI");
       return -1;
   }
...
sub ancestor {
    ...
    if (@_) {
        ...
        if ($new_ancestor) {
            $self->throw("This is [$new_ancestor], not a FAST::Bio::Tree::NodeI")
		unless $new_ancestor->isa('FAST::Bio::Tree::NodeI');
        }
...
=head2 FAST::Bio::Node::NodeI decorated interface implemented

The following methods are implemented by L<FAST::Bio::Node::NodeI> decorated
interface.
