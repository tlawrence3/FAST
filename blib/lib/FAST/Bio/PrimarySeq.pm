#
# bioperl module for FAST::Bio::PrimarySeq
#
...
=head1 SYNOPSIS
  ...
  # FAST::Bio::SeqIO for file reading, FAST::Bio::DB::GenBank for
  # database reading

  use FAST::Bio::Seq;
  use FAST::Bio::SeqIO;
  use FAST::Bio::DB::GenBank;

  # make from memory

  $seqobj = FAST::Bio::PrimarySeq->new ( -seq => 'ATGGGGTGGGCGGTGGGTGGTTTG',
                                   -id  => 'GeneFragment-12',
  ...
  $inputstream = FAST::Bio::SeqIO->new(-file => "myseq.fa",
                                 -format => 'Fasta');
...
Although new users will use FAST::Bio::PrimarySeq a lot, in general you will
be using it from the FAST::Bio::Seq object. For more information on FAST::Bio::Seq
see L<FAST::Bio::Seq>. For interest you might like to know that
FAST::Bio::Seq has-a FAST::Bio::PrimarySeq and forwards most of the function calls
...
Sequence objects are defined by the FAST::Bio::PrimarySeqI interface, and this
...
objects, then you should read the FAST::Bio::PrimarySeqI documentation

The documentation of this object is a merge of the FAST::Bio::PrimarySeq and
FAST::Bio::PrimarySeqI documentation.  This allows all the methods which you can
...
use base qw(FAST::Bio::Root::Root FAST::Bio::PrimarySeqI
            FAST::Bio::IdentifiableI FAST::Bio::DescribableI);
...
=head2 new
 ...
 Title   : new
 Usage   : $seq    = FAST::Bio::PrimarySeq->new( -seq => 'ATGGGGGTGGTGGTACCCT',
                                           -id  => 'human_id',
...
=head2 subseq
 ...
 Args    : integer for start position
           integer for end position
                 OR
           FAST::Bio::LocationI location for subseq (strand honored)
...
sub subseq {
   ...
   my $dummy = new FAST::Bio::PrimarySeq(-seq=>$replace, -alphabet=>$self->alphabet) if defined($replace);
   ...
   if( ref($start) && $start->isa('FAST::Bio::LocationI') ) {
       ...
       foreach my $subloc ($loc->each_Location()) {
           ...
           if($subloc->strand() < 0) {
               $piece = FAST::Bio::PrimarySeq->new('-seq' => $piece)->revcom()->seq();
           }
           $seq .= $piece;
       }
       return $seq;
       ...
       $self->warn("Incorrect parameters to subseq - must be two integers or a FAST::Bio::LocationI object. Got:", $self,$start,$end,$replace,$nogap);
       return;
   }
}
...
=head2 display_id
 ...
 Function: returns the display id, aka the common name of the Sequence object.
           ...
           With the new FAST::Bio::DescribeableI interface, display_name aliases
           to this method.
...
=head2 accession_number
 ...
 Function: Returns the unique biological id for a sequence, commonly
           ...
           With the new FAST::Bio::IdentifiableI interface, this is aliased
           to object_id
...
=head2 desc
 ...
 Function: Get/set description of the sequence.
           ...
           FAST::Bio::DescribeableI interface.
...
=head1 Methods Inherited from FAST::Bio::PrimarySeqI

These methods are available on FAST::Bio::PrimarySeq, although they are
actually implemented on FAST::Bio::PrimarySeqI

=head2 revcom

 Title   : revcom
 Usage   : $rev = $seq->revcom()
 Function: Produces a new FAST::Bio::SeqI implementing object which
 ...
 Returns : A new (fresh) FAST::Bio::SeqI object
 Args    : none
...
=head2 trunc
 ...
 Example :
 Returns : A fresh FAST::Bio::SeqI implementing object.
 Args    :
