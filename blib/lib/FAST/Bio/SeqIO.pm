# BioPerl module for FAST::Bio::SeqIO
#
...
=head1 SYNOPSIS
    ...
    $in  = FAST::Bio::SeqIO->new(-file => "inputfilename" ,
                           -format => 'Fasta');
    $out = FAST::Bio::SeqIO->new(-file => ">outputfilename" ,
                           -format => 'EMBL');
  ...
  # smallest reformatter
    ...
    $in  = FAST::Bio::SeqIO->newFh(-file => "inputfilename" ,
                             -format => 'Fasta');
    $out = FAST::Bio::SeqIO->newFh(-format => 'EMBL');
...
FAST::Bio::SeqIO is a handler module for the formats in the SeqIO set (eg,
FAST::Bio::SeqIO::fasta). It is the officially sanctioned way of getting at
the format objects, which most people should use.

The FAST::Bio::SeqIO system can be thought of like biological file handles.
...
can either read or write sequence objects (FAST::Bio::Seq objects, or
more correctly, FAST::Bio::SeqI implementing objects, of which FAST::Bio::Seq is
one such object). If you want to know what to do with a FAST::Bio::Seq
object, read L<FAST::Bio::Seq>.
...
the FAST::Bio::SeqIO::fasta object.
...
operations to read and write sequence objects:
    ...
    $stream = FAST::Bio::SeqIO->newFh(-format => 'Fasta',
                                -fh     => \*ARGV);
...
This makes the simplest ever reformatter
    ...
    my $in  = FAST::Bio::SeqIO->newFh(-format => $format1, -fh => \*ARGV );
    my $out = FAST::Bio::SeqIO->newFh(-format => $format2 );
...
=head2 FAST::Bio::SeqIO-E<gt>new()

   $seqIO = FAST::Bio::SeqIO->new(-file => 'filename',   -format=>$format);
   $seqIO = FAST::Bio::SeqIO->new(-fh   => \*FILEHANDLE, -format=>$format);
   $seqIO = FAST::Bio::SeqIO->new(-format => $format);

The new() class method constructs a new FAST::Bio::SeqIO object.  The
...
example, to read from STDIN:
   ...
   $seqIO = FAST::Bio::SeqIO->new(-fh => \*STDIN);
...
some HTML tags:
  ...
  use FAST::Bio::SeqIO;
  use IO::String;
  my $in  = FAST::Bio::SeqIO->new(-file => "emblfile",
  		            -format => 'EMBL');
  while ( my $seq = $in->next_seq() ) {
      # the output handle is reset for every file
      my $stringio = IO::String->new($string);
      my $out = FAST::Bio::SeqIO->new(-fh => $stringio,
  			        -format => 'fasta');
...
FAST::Bio::SeqIO::staden::read package available from the bioperl-ext
repository.
...
evaluates as defined but false:
  ...
  my $gb = FAST::Bio::SeqIO->new(-file   => "<gball.gbk",
                           -format => "gb");
  my $fa = FAST::Bio::SeqIO->new(-file   => ">gball.fa",
                           -format => "fasta",
...
=head2 FAST::Bio::SeqIO-E<gt>newFh()

   $fh = FAST::Bio::SeqIO->newFh(-fh => \*FILEHANDLE, -format=>$format);
   $fh = FAST::Bio::SeqIO->newFh(-format => $format);
   # etc.

This constructor behaves like new(), but returns a tied filehandle
rather than a FAST::Bio::SeqIO object.  You can read sequences from this
...
package FAST::Bio::SeqIO;

use strict;

use FAST::Bio::Factory::FTLocationFactory;
use FAST::Bio::Seq::SeqBuilder;
use FAST::Bio::Tools::GuessSeqFormat;
use Symbol;

use base qw(FAST::Bio::Root::Root FAST::Bio::Root::IO FAST::Bio::Factory::SequenceStreamI);
...
=head2 new
 ...
 Title   : new
 Usage   : $stream = FAST::Bio::SeqIO->new(-file => $filename,
                                     -format => 'Format')
 Function: Returns a new sequence stream
 Returns : A FAST::Bio::SeqIO stream initialised with the appropriate format
 Args    : Named parameters:
           ...
           these must be provided, they all have reasonable defaults.
             -seqfactory   the FAST::Bio::Factory::SequenceFactoryI object
             -locfactory   the FAST::Bio::Factory::LocationFactoryI object
             -objbuilder   the FAST::Bio::Factory::ObjectBuilderI object

See L<FAST::Bio::SeqIO::Handler>
...
sub new {
	...
	if( ! $format ) {
	    if ($param{-file}) {
		$format = FAST::Bio::Tools::GuessSeqFormat->new(-file => $param{-file}||$ARGV[0] )->guess;
	    } elsif ($param{-fh}) {
		$format = FAST::Bio::Tools::GuessSeqFormat->new(-fh => $param{-fh}||$ARGV[0] )->guess;
	    }
	}
	...
	return "FAST::Bio::SeqIO::$format"->new(@args);
    }
}
...
=head2 newFh
 ...
 Title   : newFh
 Usage   : $fh = FAST::Bio::SeqIO->newFh(-file=>$filename,-format=>'Format')
 Function: does a new() followed by an fh()
 Example : $fh = FAST::Bio::SeqIO->newFh(-file=>$filename,-format=>'Format')
           $sequence = <$fh>;   # read a sequence object
           print $fh $sequence; # write a sequence object
 Returns : filehandle tied to the FAST::Bio::SeqIO::Fh class
 Args    :

See L<FAST::Bio::SeqIO::Fh>
...
=head2 fh
 ...
 Example : $fh = $obj->fh;      # make a tied filehandle
           $sequence = <$fh>;   # read a sequence object
           print $fh $sequence; # write a sequence object
 Returns : filehandle tied to FAST::Bio::SeqIO class
 Args    : none
...
sub _initialize {
	...
	$locfact = FAST::Bio::Factory::FTLocationFactory->new(-verbose => $self->verbose)
	  if ! $locfact;
	$objbuilder = FAST::Bio::Seq::SeqBuilder->new(-verbose => $self->verbose)
	  unless $objbuilder;
...
=head2 next_seq
 ...
 Returns : a FAST::Bio::Seq sequence object, or nothing if no more sequences
           are available

 Args    : none

See L<FAST::Bio::Root::RootI>, L<FAST::Bio::Factory::SeqStreamI>, L<FAST::Bio::Seq>

=cut

sub next_seq {
   my ($self, $seq) = @_;
   $self->throw("Sorry, you cannot read from a generic FAST::Bio::SeqIO object.");
}
...
sub write_seq {
    my ($self, $seq) = @_;
    $self->throw("Sorry, you cannot write to a generic FAST::Bio::SeqIO object.");
}
...
sub alphabet {
   ...
   if ( defined $value) {
		...
		unless ($valid_alphabet_cache{$value}) {
			...
			eval {
				require FAST::Bio::PrimarySeq;
				my $seq = FAST::Bio::PrimarySeq->new('-verbose' => $self->verbose,
														 '-alphabet' => $value);
			};
			if ($@) {
				$self->throw("Invalid alphabet: $value\n. See FAST::Bio::PrimarySeq for allowed values.");
			}
...
sub _load_format_module {
	my ($self, $format) = @_;
	my $module = "FAST::Bio::SeqIO::" . $format;
	my $ok;
...
sub sequence_factory{
   my ($self,$obj) = @_;
   if( defined $obj ) {
		if( ! ref($obj) || ! $obj->isa('FAST::Bio::Factory::SequenceFactoryI') ) {
			$self->throw("Must provide a valid FAST::Bio::Factory::SequenceFactoryI object to ".ref($self)."::sequence_factory()");
		}
...
sub sequence_builder{
	my ($self,$obj) = @_;
	if( defined $obj ) {
		if( ! ref($obj) || ! $obj->isa('FAST::Bio::Factory::ObjectBuilderI') ) {
			$self->throw("Must provide a valid FAST::Bio::Factory::ObjectBuilderI object to ".ref($self)."::sequence_builder()");
		}
...
=head2 location_factory
 ...
 Function: Get/Set the FAST::Bio::Factory::LocationFactoryI object to be used for
...
sub location_factory{
	my ($self,$obj) = @_;
	if( defined $obj ) {
		if( ! ref($obj) || ! $obj->isa('FAST::Bio::Factory::LocationFactoryI') ) {
			$self->throw("Must provide a valid FAST::Bio::Factory::LocationFactoryI" .
