...
package FAST::Bio::Seq::SeqFastaSpeedFactory;
use strict;
...
sub create {
    ...
    my $seq = bless {}, "FAST::Bio::Seq";
    my $t_pseq = $seq->{'primary_seq'} = bless {}, "FAST::Bio::PrimarySeq";
    $t_pseq->{'seq'}  = $sequence;
    $t_pseq->{'desc'} = $fulldesc;
    $t_pseq->{'display_id'} = $id;
    $t_pseq->{'primary_id'} = $id;
    $seq->{'primary_id'} = $id; # currently FAST::Bio::Seq does not delegate this
