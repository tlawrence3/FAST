...
#
# BioPerl module for FAST::Bio::Nexml::Factory
#
...
This module handles the creation of BioPerl objects from FAST::Bio::Phylo
...
package FAST::Bio::Nexml::Factory;

use strict;

BEGIN {
    use FAST::Bio::Root::Root;
    unless (eval "require FAST::Bio::Phylo; 1") {
    FAST::Bio::Root::Root->throw("FAST::Bio::Phylo package required; see http://www.nexml.org for download details");
    }
}
    
use FAST::Bio::Phylo::Factory;
use FAST::Bio::Phylo::Matrices;
use FAST::Bio::Phylo::Matrices::Matrix;
use FAST::Bio::Phylo::Matrices::Datum;
use FAST::Bio::Phylo::Forest::Tree;
use FAST::Bio::Phylo::Matrices;
use FAST::Bio::Phylo::IO;
...
my $fac = FAST::Bio::Phylo::Factory->new();
        ...
        while ($row)
        {
            ...
            if($taxa)
            {
                if (my $taxon = $taxa->get_by_name($row->get_taxon->get_name())) {
                    ...
                    $seq_feats = FAST::Bio::SeqFeature::Generic->new();
...
sub create_bperl_tree {
    ...
    {   
        ...
        {                       
            ...
            my $tree = FAST::Bio::Tree::Tree->new(-id => "$basename.$tree_id");
            ...
            # TODO: should $caller->{_ID} always be defined?
            # ATM, this is a FAST::Bio::TreeIO::nexml stream...            
...
#               my $terminal = $$terminals[$i];
                ...
                {
                    $newNode = FAST::Bio::Tree::Node->new();
                ...
                {
                    my $parent_node = FAST::Bio::Tree::Node->new();
...
sub create_bperl_seq {
    ...
    {   
        ...
        {
            my $newSeq = $row->get_char();
            my $feat = FAST::Bio::SeqFeature::Generic->new();
            ...
            my $seqbuilder = new FAST::Bio::Seq::SeqFactory('-type' => 'FAST::Bio::Seq');
            ...
            # TODO: should $caller->{_ID} always be defined?
            # ATM, this is a FAST::Bio::SeqIO::nexml stream...            
...
sub create_bphylo_tree {
    ...
    #most of the code below ripped form FAST::Bio::Phylo::Forest::Tree::new_from_bioperl()d
    
    my $tree = $fac->create_tree;
    my $class = 'FAST::Bio::Phylo::Forest::Tree';
    
    if ( ref $bptree && $bptree->isa('FAST::Bio::Tree::TreeI') ) {
        bless $tree, $class;
...
sub create_bphylo_node {
    my ($bpnode) = @_;
        my $node = FAST::Bio::Phylo::Forest::Node->new();
        
        #mostly ripped from FAST::Bio::Phylo::Forest::Node->new_from_bioperl()
        # copy name
        my $name = $bpnode->id;
...
sub create_bphylo_aln {
    ...
    #most of the code below ripped from FAST::Bio::Phylo::Matrices::Matrix::new_from_bioperl()
    if ( $aln->isa('FAST::Bio::Align::AlignI') ) {
            $aln->unmatch;
            $aln->map_chars('\.','-');
...
sub create_bphylo_seq {
    ...
    if ( $seqstring and $seqstring =~ /\S/ ) {
        ...
        if ( $@ and UNIVERSAL::isa($@,'FAST::Bio::Phylo::Util::Exceptions::InvalidData') ) {
...
sub create_bphylo_taxa {
    ...
    if ( UNIVERSAL::isa( $obj, 'FAST::Bio::Align::AlignI' ) || UNIVERSAL::isa( $obj, 'FAST::Bio::Seq')) {
        return $self->_create_bphylo_matrix_taxa(@_);
    }
    elsif ( UNIVERSAL::isa( $obj, 'FAST::Bio::Tree::TreeI' ) ) {
...
sub create_bphylo_datum {
    #mostly ripped from FAST::Bio::Phylo::Matrices::Datum::new_from_bioperl()
    my ( $seq, $taxa, @args ) = @_;
    my $class = 'FAST::Bio::Phylo::Matrices::Datum';
    my $feats;
    # want $seq type-check here? Allowable: is-a FAST::Bio::PrimarySeq, 
        #  FAST::Bio::LocatableSeq /maj
    if (@args % 2) { # odd
        $feats = shift @args;
        unless (ref($feats) eq 'ARRAY') {
        FAST::Bio::Root::Root->throw("Third argument must be array of SeqFeatures");
        }
    }
        my $type = $seq->alphabet || $seq->_guess_alphabet || 'dna';
    ...
    # only FAST::Bio::LocatableSeq objs have these fields...
        for my $field ( qw(start end strand) ) {
