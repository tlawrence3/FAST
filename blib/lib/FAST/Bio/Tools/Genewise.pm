#
# BioPerl module for FAST::Bio::Tools::Genewise
#
# Copyright Fugu Team 
#
...
=head1 SYNOPSIS
  ...
  use FAST::Bio::Tools::Genewise;
  my $gw = FAST::Bio::Tools::Genewise(-file=>"genewise.out");
...
FAST::Bio::SeqFeature::Gene::GeneStructure object.
...
package FAST::Bio::Tools::Genewise;
use vars qw($Srctag);
use strict;
use Symbol;

use FAST::Bio::Tools::AnalysisResult;
use FAST::Bio::SeqFeature::Generic;
use FAST::Bio::SeqFeature::Gene::Exon;
use FAST::Bio::SeqFeature::FeaturePair;
...
=head2 new
 ...
 Example :
 Returns : FAST::Bio::Tools::Genewise object

See L<FAST::Bio::Tools::Genewise>
...
=head2 next_prediction
 ...
 Example :
 Returns : a FAST::Bio::SeqFeature::Gene::GeneStructure object
 Args    :

See L<FAST::Bio::SeqFeature::Gene::GeneStructure>
...
sub _parse_genes {
	...
	foreach my $gene_txt (@genes_txt) {
	    ...
	    my $genes = FAST::Bio::SeqFeature::Gene::GeneStructure->new
		(-source => $source_tag,
		 -score  => $self->_score,
		 );
		...
		my $exon = FAST::Bio::SeqFeature::Gene::Exon->new
		...
		if( $e =~ m/Supporting\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)/) {
		    ...
		    my $pf = FAST::Bio::SeqFeature::Generic->new
			( -start   => $prot_start, 
			  -end     => $prot_end,
		    ...
		    my $gf = FAST::Bio::SeqFeature::Generic->new 
			( -start   => $geno_start,
			  -end     => $geno_end,
			  -seq_id  => $self->_target_id,
			  -score   => $score,
		    ...
		    my $fp = FAST::Bio::SeqFeature::FeaturePair->new
			(-feature1 =>$gf,
			 -feature2 =>$pf);
