...
#     EXE_FILES => [q[bin/alndegap], q[bin/alnpi], q[bin/alnseg], q[bin/fascodon], q[bin/fascomp], q[bin/fasconvert], q[bin/fascut], q[bin/fasfilter], q[bin/fasgrep], q[bin/faslen], q[bin/fasrc], q[bin/fassub], q[bin/fastr], q[bin/fasuniq], q[bin/fasxl], q[bin/fassort], q[bin/fastaxsort], q[bin/fastax], q[bin/gbfeat2fas], q[bin/gbalncut], q[bin/gbgrep]]
...
EXE_FILES = bin/alndegap bin/alnpi bin/alnseg bin/fascodon bin/fascomp bin/fasconvert bin/fascut bin/fasfilter bin/fasgrep bin/faslen bin/fasrc bin/fassub bin/fastr bin/fasuniq bin/fasxl bin/fassort bin/fastaxsort bin/fastax bin/gbfeat2fas bin/gbalncut bin/gbgrep

pure_all :: $(INST_SCRIPT)/alndegap $(INST_SCRIPT)/alnseg $(INST_SCRIPT)/fastaxsort $(INST_SCRIPT)/faslen $(INST_SCRIPT)/gbfeat2fas $(INST_SCRIPT)/alnpi $(INST_SCRIPT)/fasconvert $(INST_SCRIPT)/gbgrep $(INST_SCRIPT)/fascodon $(INST_SCRIPT)/fasuniq $(INST_SCRIPT)/fasfilter $(INST_SCRIPT)/fasxl $(INST_SCRIPT)/fastax $(INST_SCRIPT)/gbalncut $(INST_SCRIPT)/fassort $(INST_SCRIPT)/fascut $(INST_SCRIPT)/fasrc $(INST_SCRIPT)/fassub $(INST_SCRIPT)/fastr $(INST_SCRIPT)/fasgrep $(INST_SCRIPT)/fascomp
	$(NOECHO) $(NOOP)

realclean ::
	$(RM_F) \
	  $(INST_SCRIPT)/alndegap $(INST_SCRIPT)/alnseg \
	  $(INST_SCRIPT)/fastaxsort $(INST_SCRIPT)/faslen \
	  $(INST_SCRIPT)/gbfeat2fas $(INST_SCRIPT)/alnpi \
	  $(INST_SCRIPT)/fasconvert $(INST_SCRIPT)/gbgrep \
	  $(INST_SCRIPT)/fascodon $(INST_SCRIPT)/fasuniq \
	  $(INST_SCRIPT)/fasfilter $(INST_SCRIPT)/fasxl \
	  $(INST_SCRIPT)/fastax $(INST_SCRIPT)/gbalncut \
	  $(INST_SCRIPT)/fassort $(INST_SCRIPT)/fascut \
	  $(INST_SCRIPT)/fasrc $(INST_SCRIPT)/fassub \
