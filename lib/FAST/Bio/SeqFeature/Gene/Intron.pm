#
# BioPerl module for FAST::Bio::SeqFeature::Gene::Intron
#
...
package FAST::Bio::SeqFeature::Gene::Intron;
use strict;

use FAST::Bio::SeqFeature::Gene::Exon;

use base qw(FAST::Bio::SeqFeature::Gene::NC_Feature);
...
sub upstream_Exon {
    ...
    if ($exon) {
        ...
        $self->throw("'$exon' is not a FAST::Bio::SeqFeature::Gene::ExonI") 
	    unless $exon->isa('FAST::Bio::SeqFeature::Gene::ExonI');
...
sub acceptor_splice_site {
  ...
  if($self->strand < 0){
      ...
      $acceptor = FAST::Bio::SeqFeature::Generic->new(-start=>$self->start - ($ss_start) ,  
    ...
    $acceptor = FAST::Bio::SeqFeature::Generic->new(-start=>$self->end - ($ss_start + 1),  
                                               -end=>$self->end + $ss_end,
                                               -strand=>$self->strand,    
                                               -primary_tag=>"donor splice site");
  }
...
sub donor_splice_site {
  ...
  if($self->strand < 0){
    ...
    $ss_start   = $ss_start> $self->length -2 ? $self->length -2 : $ss_start;
    $donor = FAST::Bio::SeqFeature::Generic->new(-start=>$self->end -  ($ss_start+1),
                                            -end  => $self->end + ($ss_end),
                                            -strand=>$self->strand,
    ...
    $ss_end   = $ss_end > $self->length -2 ? $self->length -2 : $ss_end;
    $donor = FAST::Bio::SeqFeature::Generic->new(-start=>$self->start - $ss_start,
                                           -end  => $self->start +($ss_end+1),
                                            -strand=>$self->strand,
                                            -primary_tag=>"acceptor splice site");
  }
...
sub location {
    ...
    unless ($self->{'_intron_location'}) {
        my $loc = FAST::Bio::Location::Simple->new;
