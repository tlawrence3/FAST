#
# BioPerl module for FAST::Bio::SeqFeature::Generic
#
...
=head1 SYNOPSIS
   ...
   $feat = FAST::Bio::SeqFeature::Generic->new( 
            -start        => 10, 
            -end          => 100,
            -strand       => -1, 
   ...
   $feat = FAST::Bio::SeqFeature::Generic->new( -gff_string => $string );
   # if you want explicitly GFF1
   $feat = FAST::Bio::SeqFeature::Generic->new( -gff1_string => $string );
...
FAST::Bio::SeqFeature::Generic is a generic implementation for the
FAST::Bio::SeqFeatureI interface, providing a simple object to provide all
...
is what is done in the L<FAST::Bio::SeqFeature::Gene>,
L<FAST::Bio::SeqFeature::Transcript> and L<FAST::Bio::SeqFeature::Exon>, which provide
...
L<FAST::Bio::SeqFeature::Generic> programmatically: that is, you know that you have
...
format via the L<FAST::Bio::SeqIO> system
...
=item L<FAST::Bio::SeqFeatureI>

Note that this includes implementing FAST::Bio::RangeI.

=item L<FAST::Bio::AnnotatableI>

=item L<FAST::Bio::FeatureHolderI>
...
package FAST::Bio::SeqFeature::Generic;
use strict;

use FAST::Bio::Annotation::Collection;
use FAST::Bio::Location::Simple;
use FAST::Bio::Location::Split;
use FAST::Bio::Tools::GFF;
#use Tie::IxHash;

use base qw(FAST::Bio::Root::Root FAST::Bio::SeqFeatureI FAST::Bio::FeatureHolderI FAST::Bio::AnnotatableI);
...
sub set_attributes {
    ...
    $gff1_string  && do {
        $self->gff_format(FAST::Bio::Tools::GFF->new('-gff_version' => 1));
...
=head2 direct_new
 ...
 Title   : direct_new
 Usage   : my $obj = FAST::Bio::SeqFeature::Generic->direct_new
 Function: create a blessed hash - for performance improvement in 
           object creation
 Returns : FAST::Bio::SeqFeature::Generic object
 Args    : none
...
=head2 location
 ...
 Args    : [optional] FAST::Bio::LocationI object to set the value to.
...
sub attach_seq {
   ...
   if ( ! ($seq && ref($seq) && $seq->isa("FAST::Bio::PrimarySeqI")) ) {
       $self->throw("Must attach FAST::Bio::PrimarySeqI objects to SeqFeatures");
   }
...
=head2 annotation
 ...
 Example : 
 Returns : A FAST::Bio::AnnotationCollectionI object
 Args    : newvalue (optional)
...
sub annotation {
    ...
    if(defined $value || ! defined $obj->{'annotation'} ) {
        $value = FAST::Bio::Annotation::Collection->new() unless ( defined $value );
...
sub add_SeqFeature{
    ...
    if ( !$feat->isa('FAST::Bio::SeqFeatureI') ) {
        $self->warn("$feat does not implement FAST::Bio::SeqFeatureI. Will add it anyway, but beware...");
    }
...
=head2 remove_SeqFeatures
 ...
 Example :
 Returns : The array of FAST::Bio::SeqFeatureI implementing features that was
           deleted.
 Args    : none
...
=head2 gff_format
 ...
 Usage   : # get:
           ...
           $feature->gff_format(FAST::Bio::Tools::GFF->new(-gff_version => 1));
 Function: Get/set the GFF format interpreter. This object is supposed to 
           format and parse GFF. See FAST::Bio::Tools::GFF for the interface.
 ...
 Example : 
 Returns : a FAST::Bio::Tools::GFF compliant object
 Args    : On set, an instance of FAST::Bio::Tools::GFF or a derived object.
...
sub gff_format {
    ...
    if(defined($gffio)) {
        if(ref($self)) {
            ...
            $FAST::Bio::SeqFeatureI::static_gff_formatter = $gffio;
        }
    }
...
=head2 gff_string
 ...
 Function: Provides the feature information in GFF format.
           ...
           We override this here from FAST::Bio::SeqFeatureI in order to use the
...
=head2 slurp_gff_file
 ...
 Title   : slurp_file
 Usage   : @features = FAST::Bio::SeqFeature::Generic::slurp_gff_file(\*FILE);
 Function: Sneaky function to load an entire file as in memory objects.
           Beware of big files.

           This method is deprecated. Use FAST::Bio::Tools::GFF instead, which can
...
sub slurp_gff_file {
   ...
   my @out;
   if ( !defined $f ) {
       FAST::Bio::Root::Root->throw("Must have a filehandle");
   }

   FAST::Bio::Root::Root->deprecated( -message => "deprecated method slurp_gff_file() called in FAST::Bio::SeqFeature::Generic. Use FAST::Bio::Tools::GFF instead.",
                                -warn_version  => '1.005',
                                -throw_version => '1.007',
                              );

   while(<$f>) {
       my $sf = FAST::Bio::SeqFeature::Generic->new('-gff_string' => $_);
       push(@out, $sf);
   }
...
=head2 _expand_region
 ...
 Returns : 
 Args    : A FAST::Bio::SeqFeatureI implementing object.
...
sub _expand_region {
    my ($self, $feat) = @_;
    if(! $feat->isa('FAST::Bio::SeqFeatureI')) {
        $self->warn("$feat does not implement FAST::Bio::SeqFeatureI");
    }
