#
# BioPerl module for FAST::Bio::SeqIO::chaos
#
...
This is the guts of L<FAST::Bio::SeqIO::chaosxml> - please refer to the
...
probably want to use L<FAST::Bio::SeqIO::chaosxml>, which is a wrapper to
this module.
...
end-product xml, use L<FAST::Bio::SeqIO::chaosxml>
...
use FAST::Bio::SeqFeature::Tools::TypeMapper;
use FAST::Bio::SeqFeature::Tools::FeatureNamer;
use FAST::Bio::SeqFeature::Tools::IDHandler;
use Data::Stag qw(:all);

use base qw(FAST::Bio::SeqIO);

our $TM = 'FAST::Bio::SeqFeature::Tools::TypeMapper';
our $FNAMER = 'FAST::Bio::SeqFeature::Tools::FeatureNamer';
our $IDH = 'FAST::Bio::SeqFeature::Tools::IDHandler';
...
sub _initialize {
    ...
    $self->handler->ev(chaos_metadata=>[
                                        ...
                                        [export_program=>$0],
                                        [export_module=>'FAST::Bio::SeqIO::chaos'],
...
sub write_sf {
    ...
    if($CREATE_SPLIT_SFS &&
       $loc->isa("FAST::Bio::Location::SplitLocationI") ) {
    ...
    elsif( $loc->isa("FAST::Bio::Location::RemoteLocationI") ) {
        # turn splitlocs into subfeatures
        my $n = 1;
        push(@subsfs,
             map {
...
sub get_chaos_feature_id {
    ...
    my $id;
    if ($ob->isa("FAST::Bio::SeqI")) {
        $id = $ob->accession_number . '.' . ($ob->can('seq_version') ? $ob->seq_version : $ob->version);
    }
    else {
        $ob->isa("FAST::Bio::SeqFeatureI") || $self->throw("$ob must be either SeqI or SeqFeatureI");
    ...
    if (!$id) {
        if ($ob->isa("FAST::Bio::SeqFeatureI")) {
            $id = $IDH->generate_unique_persistent_id($ob);
        }
        else {
