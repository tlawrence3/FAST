#
# BioPerl module for FAST::Bio::SeqIO::kegg
#
...
FAST::Bio::Seq::RichSeqI implementing object returned by the parser for each
record.
...
sub next_seq {
	...
	$annotation = FAST::Bio::Annotation::Collection->new();

	$annotation->add_Annotation('description',
						FAST::Bio::Annotation::Comment->new(-text => $definition));

	$annotation->add_Annotation('aa_seq',
						FAST::Bio::Annotation::Comment->new(-text => $aa_seq));
	...
	if ($FIELDS{ORTHOLOG}) {
        ...
        $annotation->add_Annotation('dblink',FAST::Bio::Annotation::DBLink->new(
  ...
  if($FIELDS{PATHWAY}) {
     ...
     while($FIELDS{PATHWAY} =~ /\s*PATH:\s+(.+)$/mg){
        $annotation->add_Annotation('pathway',
           FAST::Bio::Annotation::Comment->new(-text => "$1"));
     }
  }
  ...
  if($FIELDS{POSITION}) {
    ...
    $annotation->add_Annotation('position',
      FAST::Bio::Annotation::Comment->new(-text => $FIELDS{POSITION}));
  }
  ...
  if ($FIELDS{CLASS}) {
      ...
      while($FIELDS{CLASS} =~ /(.*?)\[(\S+):(\S+)\]/g){
          ...
          $pathway =~ s/^\s+//;
          $annotation->add_Annotation('pathway',
                  FAST::Bio::Annotation::Comment->new(-text => $pathway));
  ...
  $params{'-species'}          = FAST::Bio::Species->new(
