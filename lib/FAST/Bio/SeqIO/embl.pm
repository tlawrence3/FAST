#
# BioPerl module for FAST::Bio::SeqIO::EMBL
#
...
rather go through the SeqIO handler system. Go:
    ...
    $stream = FAST::Bio::SeqIO->new(-file => $filename, -format => 'EMBL');
...
This object can transform FAST::Bio::Seq objects to and from EMBL flat
file databases.
    ...
    BEFORE_FEATURE_TABLE :
          ...
          until ( !defined $buffer ) {
              ...
              # Comments
                  ...
                  my $commobj = FAST::Bio::Annotation::Comment->new();
...
=head2 _write_ID_line
 ...
 Function: Writes the EMBL Release 87 format ID line to the stream, unless
         ...
         : ( See FAST::Bio::SeqIO::embl::_id_generation_function(). )
 Returns : nothing
...
sub write_seq {
    ...
    foreach my $seq ( @seqs ) {
        ...
        unless ( ref $seq && $seq->isa('FAST::Bio::SeqI' ) ) {
            $self->warn("$seq is not a SeqI compliant sequence object!")
                if $self->verbose >= 0;
            unless ( ref $seq && $seq->isa('FAST::Bio::PrimarySeqI' ) ) {
        ...
        my( $acc );
        {
            if ( my $func = $self->_ac_generation_func ) {
                $acc = &{$func}($seq);
            } elsif ( $seq->isa('FAST::Bio::Seq::RichSeqI') &&
...
sub _read_EMBL_Contig {
	...
	/^C/ || last;
	/^CO\s+(.*)/ && do {
	push @ret, FAST::Bio::Annotation::SimpleValue->new( -tagname => 'contig',
						      -value => $1);
	};
    }
    $$buffer = $_;
...
sub _read_EMBL_References {
    ...
    my $ref = FAST::Bio::Annotation::Reference->new();
    $au =~ s/;\s*$//g;
    $title =~ s/;\s*$//g;
...
#    $$buffer = $_;
    ...
    # do minimal sanity checks before we hand off to FAST::Bio::Species which won't
...
=head2 _read_EMBL_DBLink
 ...
 Example :
 Returns : A list of FAST::Bio::Annotation::DBLink objects
 Args    :
...
sub _read_EMBL_DBLink {
    ...
    while (defined( $_ ||= $self->_readline )) {
        ...
        my $link = FAST::Bio::Annotation::DBLink->new(-database    => $databse,
                            -primary_id  => $prim_id,
                            -optional_id => $sec_id);
