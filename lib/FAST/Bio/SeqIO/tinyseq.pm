# BioPerl module for FAST::Bio::SeqIO::tinyseq
#
...
Do not use this module directly; use the SeqIO handler system:
  ...
  $stream = FAST::Bio::SeqIO->new( -file => $filename, -format => 'tinyseq' );
...
This object reads and writes FAST::Bio::Seq objects to and from TinySeq XML
...
L<FAST::Bio::SeqIO>, L<FAST::Bio::Seq>.
...
McKay's L<FAST::Bio::SeqIO::game>.  If it breaks, however, it's my fault not
his ;).
...
package FAST::Bio::SeqIO::tinyseq;

use strict;
use FAST::Bio::Seq::SeqFastaSpeedFactory;
use FAST::Bio::Species;
use FAST::Bio::SeqIO::tinyseq::tinyseqHandler;
...
sub _initialize {
	...
	$self->sequence_factory(FAST::Bio::Seq::SeqFastaSpeedFactory->new());
    }
...
sub write_seq {
    ...
    $self->throw('write_seq must be called with at least one  FAST::Bio::SeqI or FAST::Bio::PrimarySeqI compliant object')
	unless (@seqobjs and ( $seqobjs[0]->isa('FAST::Bio::SeqI') || $seqobjs[0]->isa('FAST::Bio::PrimarySeqI')));
...
=head2 _get_seqs
  ...
  Function	: parses the XML and creates FAST::Bio::Seq objects
  Returns	: 1 on success
  Args		: NONE
...
sub _get_seqs {
    ...
    my $handler = FAST::Bio::SeqIO::tinyseq::tinyseqHandler->new();
...
# 		my $species = FAST::Bio::Species->new( -classification	=> $class,
...
=head2 _get_species
  ...
  Function	: gets a FAST::Bio::Species object from cache or creates as needed
  Returns	: a FAST::Bio::Species object on success, undef on failure
...
=head2 _create_species
  ...
  Usage		: Internal function
  Function	: creates a FAST::Bio::Species object
  Returns	: a FAST::Bio::Species object on success, undef on failure
  Args		: a classification string (eg 'Homo sapiens') and
                  a NCBI taxon id (optional)
...
sub _create_species {
    ...
    my $species = FAST::Bio::Species->new(%params)
	or return;
...
=head2 _convert_seqtype
  ...
  Usage		: Internal function
  Function	: maps FAST::Bio::Seq::alphabet() values [dna/rna/protein] onto
...
sub _get_writer {
	...
	$writer->comment("Generated by FAST::Bio::SeqIO::tinyseq VERSION ".$FAST::Bio::SeqIO::tinyseq::VERSION);
