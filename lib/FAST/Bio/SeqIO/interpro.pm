...
=head1 SYNOPSIS
  ...
  # do not call this module directly, use FAST::Bio::SeqIO

  use strict;
  use FAST::Bio::SeqIO;

  my $io = FAST::Bio::SeqIO->new(-format => "interpro",
                           -file   => $interpro_file);
...
L<FAST::Bio::SeqIO::interpro> will parse Interpro scan XML (version 1.2) and
create L<FAST::Bio::SeqFeature::Generic> objects based on the contents of the
XML document.

L<FAST::Bio::SeqIO::interpro> will also attach the annotation given in the XML
file to the L<FAST::Bio::SeqFeature::Generic> objects that it creates.
...
package FAST::Bio::SeqIO::interpro;
use strict;
...
sub next_seq {
	...
	for(my $interpn=0; $interpn<scalar(@interproNodes); $interpn++){
		...
		for(my $match=0; $match<scalar(@matchNodes); $match++){
			...
			my @seqFeatures = map { FAST::Bio::SeqFeature::Generic->new(
			...
			foreach my $seqFeature (@seqFeatures){
				...
				my $annotation2 = FAST::Bio::Annotation::DBLink->new;
				...
				# Bug 1908 (enhancement)
 				my $annotation3  = FAST::Bio::Annotation::DBLink->new;
  				...
  				$seqFeature->annotation->add_Annotation('dblink',$annotation3);
                                # need to put in the go annotation here!
                                 foreach my $g (@goNodes)
                                 {
                                     ...
                                     my $go_annotation = FAST::Bio::Annotation::DBLink->new;
...
sub _initialize {
  ...
  $self->_sequence_factory( FAST::Bio::Seq::SeqFactory->new
