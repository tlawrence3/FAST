# BioPerl module for FAST::Bio::SeqIO::seqxml
#
...
This object can transform FAST::Bio::Seq objects to and from SeqXML format.
...
FAST::Bio::TreeIO::phyloxml by Mira Han.
...
sub write_seq {
    ...
    foreach my $seqobj (@seqs) {
        ...
        if ( !ref $seqobj || !$seqobj->isa('FAST::Bio::SeqI') ) {
            $self->warn(
...
sub element_species {
    ...
    {
        $species_obj =
          FAST::Bio::Species->new( -ncbi_taxid => $species_data->{'ncbiTaxID'}, );
...
sub element_DBRef {
    ...
    if (   defined $DBRef->{'source'}
        ...
        && defined $DBRef->{'type'})
    {
        $annotation_obj = FAST::Bio::Annotation::DBLink->new(
            -primary_id => $DBRef->{'id'},
...
sub element_property {
    ...
    if ( defined $property->{'name'} ) {
        $annotation_obj =
          FAST::Bio::Annotation::SimpleValue->new( -tagname => $property->{'name'} );
...
sub end_element_entry {
    ...
    if ( $data->{'source'} ) {
        my $annotation_obj = FAST::Bio::Annotation::SimpleValue->new(
            '-tagname' => 'source',
            '-value'   => $data->{'source'},
        );
        $seq_obj->add_Annotation($annotation_obj);
