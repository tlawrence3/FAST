...
Do not use this module directly.  Use it via the L<FAST::Bio::SeqIO> class.
...
sub next_seq {
    ...
    while (my $entry = $self->_readline) {
        ...
        if ($entry =~ /^BEGIN_SEQUENCE\s+(\S+)/) {
            ...
            my $collection = FAST::Bio::Annotation::Collection->new;
            while ($entry = $self->_readline) {
                ...
                if ($entry =~ /^(\w+):\s+(.+)$/) {
                    ...
                    $seq->FAST::Bio::Seq::RichSeq::annotation($collection);
                    last;
                }
            }
...
sub write_seq {
    ...
    $self->throw("You must pass a FAST::Bio::Seq::Quality object to write_seq")
        unless (ref($swq) eq "FAST::Bio::Seq::Quality");
...
sub FAST::Bio::Seq::Quality::attribute {
    ...
    unless (defined $attribute{$name}) { #new comment
        my $comment =
                FAST::Bio::Annotation::Comment->new(-text => $value || 'unknown');
...
sub FAST::Bio::Seq::Quality::chromat_file {
    my ($self,$arg) =  @_;
...
sub FAST::Bio::Seq::Quality::abi_thumbprint {
    my ($self,$arg) =  @_;
...
sub FAST::Bio::Seq::Quality::phred_version {
    my ($self,$arg) =  @_;
...
sub FAST::Bio::Seq::Quality::call_method {
    my ($self,$arg) =  @_;
...
sub FAST::Bio::Seq::Quality::quality_levels {
    my ($self,$arg) =  @_;
...
sub FAST::Bio::Seq::Quality::trace_array_min_index {
    my ($self,$arg) =  @_;
...
sub FAST::Bio::Seq::Quality::trace_array_max_index {
    my ($self,$arg) =  @_;
...
sub FAST::Bio::Seq::Quality::touch {
    my $time = localtime();
