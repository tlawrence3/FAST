#
# BioPerl module for FAST::Bio::SeqIO::genbank
#
...
package FAST::Bio::SeqIO::genbank;
use strict;
  ...
  RECORDSTART:
	...
	until( !defined ($buffer) ) {
		...
		# Project
			...
			if ($annotation) {
				my $project = new FAST::Bio::Annotation::SimpleValue->new(-value => $1);
		...
		if($annotation) {
		    ...
		    $annotation->add_Annotation('comment',
						FAST::Bio::Annotation::Comment->new(-text => $comment,
									      -tagname => 'comment'));
		    $buffer = $_;
		} else {
		    while(defined($buffer = $self->_readline())) {
...
#		commented out until kinks get worked out
	...
	if( defined ($_) ) {
		...
		if ($ctg) {
		    $annotation->add_Annotation( 
			FAST::Bio::Annotation::SimpleValue->new(-tagname => 'contig',
							  -value => $ctg )
			);
		}
		$self->_pushback($_);
		...
		while($_ =~ s/(^WGS|WGS_SCAFLD)\s+//){ # gulp lines
		    chomp;
		    $annotation->add_Annotation(
						FAST::Bio::Annotation::SimpleValue->new(-value => $_,
										  -tagname => lc($1)));
		    $_ = $self->_readline;
		}
	...
	if($builder->want_slot('seq')) {
		...
		if( $annotation && length($_) > 0 ) {
		    $annotation->add_Annotation('origin',
						FAST::Bio::Annotation::SimpleValue->new(-tagname => 'origin',
										  -value => $_));
		}
		my $seqc = '';
		while( defined($_ = $self->_readline) ) {
