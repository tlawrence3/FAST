#
# BioPerl module for FAST::Bio::SeqIO::chadoxml
#
...
rather go through the SeqIO handler system:
    ...
    $writer = FAST::Bio::SeqIO->new(-file => ">chado.xml",
                              -format => 'chadoxml');
...
This object can transform FAST::Bio::Seq objects to chadoxml flat
...
This is currently a write-only module.
    ...
    $seqio = FAST::Bio::SeqIO->new(-file => '>outfile.xml',
                             -format => 'chadoxml'
                             -suppress_residues => 1,
                             -allow_residues => 'chromosome',
                             );

    # we have a FAST::Bio::Seq object $seq which is a gene located on
    ...
    # feature to be passed into FAST::Bio::SeqIO::chadoxml->write_seq()
...
The chadoxml output of FAST::Bio::SeqIO::chadoxml-E<gt>write_seq() method can be
...
In converting a FAST::Bio::Seq object into chadoxml, a top-level feature is
created to represent the object and all sequence features inside the
FAST::Bio::Seq object are treated as subfeatures of the top-level
feature. The FAST::Bio::SeqIO::chadoxml object calls
FAST::Bio::SeqFeature::Tools::Unflattener to unflatten the flat feature list
contained in the subject FAST::Bio::Seq object, to build gene model
containment hierarchy conforming to chado central dogma model: gene
--E<gt> mRNA --E<gt> exons and protein.

Destination of data in the subject FAST::Bio::Seq object $seq is as following:
...
package FAST::Bio::SeqIO::chadoxml;
use strict;
use English;
...
use FAST::Bio::SeqIO::FTHelper;
use FAST::Bio::Species;
use FAST::Bio::Seq::SeqFactory;
use FAST::Bio::Factory::SequenceStreamI;
...
#global variables
undef(my %finaldatahash); #data from FAST::Bio::Seq object stored in a hash
undef(my %datahash); #data from FAST::Bio::Seq object stored in a hash
...
=head2 write_seq
 ...
 Args     : A FAST::Bio::Seq object $seq, optional $seqSOtype, $srcfeature,
...
sub write_seq {
    my $usage = <<EOUSAGE;
FAST::Bio::SeqIO::chadoxml->write_seq()
Usage   : \$stream->write_seq(-seq=>\$seq,
                  ...
                  -nounflatten=>0 or 1,
                              -is_analysis=>'true' or 'false',
                              -data_source=>\$datasource)
Args    : \$seq     : a FAST::Bio::Seq object
...
EOUSAGE
    ...
    if( ! ref $seq || ! $seq->isa('FAST::Bio::Seq::RichSeqI') ) {
...
#   my $miniref= undef;
    ...
    my %ftype_hash = $self->return_ftype_hash($ftype);
        ...
        if (!$self->suppress_residues ||
            ...
            $residues = $seq->seq->isa('FAST::Bio::PrimarySeq')
                        ? $seq->seq->seq
                        : $seq->seq;
        }
        else {
    ...
    #if $srcfeature is not given, use the FAST::Bio::Seq object itself as the srcfeature for featureloc's
    if (!defined $srcfeature) {
    ...
    if ($seq->can('annotation')) {
        ...
        my @top_dbxrefs = ();
        #feature object from FAST::Bio::DB::SeqFeature::Store
    ...
    if( $seq->isa('FAST::Bio::Seq::RichSeqI') && defined $seq->get_secondary_accessions() ) {
    ...
    #GI number
    if( $seq->isa('FAST::Bio::Seq::RichSeqI') && defined ($seq->pid)) {
        my $id = $seq->pid;
    ...
    if (($gb_type eq 'gene' || $gb_type eq 'DNA') &&
        !$nounflatten) {
        my $u = FAST::Bio::SeqFeature::Tools::Unflattener->new;
...
sub _subfeat2featrelhash {
    ...
    if ($seqtype eq 'mRNA' && $sftype eq 'gene') {
        ...
        if ($feat->location->isa('FAST::Bio::Location::FuzzyLocationI')) {
...
sub _getRefAuthors {
    ...
    #no authors, FAST::Bio::SeqIO::genbank doesn't pick up 'CONSRTM' line.
    else {
        return;
    }
