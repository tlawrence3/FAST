# BioPerl module: FAST::Bio::SeqIO::agave
#
...
rather go through the SeqIO handler system. Go:
  ...
  $in  = FAST::Bio::SeqIO->new('-file'   => "$file_in",
                         '-format' => 'EMBL');

  $out = FAST::Bio::SeqIO->new('-file'   => ">$file_out",
                         '-format' => 'AGAVE');
...
This object can transform FAST::Bio::Seq objects to agave xml file and
        ...
        for my $contig (@{$sciobj->{'contig'}}) { # Each contig has a fragment order.
            ...
            for my $fragment_order (@{$contig->{'fragment_order'}}) { # Each fragment order has a fragment_orientation.
                ...
                for my $fragment_orientation (@{$fragment_order->{'fragment_orientation'}}) {
                    ...
                    my $primary_seq = FAST::Bio::PrimarySeq->new(
                                                           -id       => $accession_number,
                    ...
                    for my $keywords (@$keywords) {
                        ...
                        my $reference = FAST::Bio::Annotation::Reference->
                        ...
                        for my $sequence_map (@{$bio_sequence->{'sequence_map'}}) {
                            ...
                            if (defined $sequence_map->{annotations} &&
                                ...
                                for my $seq_feature (@{$sequence_map->{'annotations'}->{'seq_feature'}}) {
                                    ...
                                    my $feat = FAST::Bio::SeqFeature::Generic->
                                        new(
                                            -start       => $start_coord,
                                            -end         => $end_coord,
...
sub next_seq {
    ...
    $self->throw("Error: No FAST::Bio::Seq objects stored yet!\n\n")
