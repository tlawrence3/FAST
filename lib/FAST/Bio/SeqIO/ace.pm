#
# BioPerl module for FAST::Bio::SeqIO::ace
#
...
This object can transform FAST::Bio::Seq objects to and
...
sub _initialize {
  ...
  if( ! defined $self->sequence_factory ) {
      $self->sequence_factory(FAST::Bio::Seq::SeqFactory->new(-verbose => $self->verbose(), -type => 'FAST::Bio::PrimarySeq'));
  }
}
...

{
    ...
    sub next_seq {
        ...
        my $mol_type = $bio_mol_type{lc $type}
            or $self->throw("Can't get FAST::Bio::Seq molecule type for '$type'");
	...
	$id =~ s/\\([\/"%;\t\\])/$1/g;
#"
	# Called as next_seq(), so give back a FAST::Bio::Seq
