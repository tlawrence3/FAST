# 
#
# Helper module for FAST::Bio::SeqIO::game::featHandler
#
...
FAST::Bio::SeqIO::game::featHandler converts game XML E<lt>annotationE<gt>
elements into flattened FAST::Bio::SeqFeature::Generic objects to be added
to the sequence
...
package FAST::Bio::SeqIO::game::featHandler;
...
sub add_source {
    ...
    my $feat = FAST::Bio::SeqFeature::Generic->new( -primary => 'source',
					      -start   => 1,
					      -end     => $length,
					    );
    for ( keys %{$tags} ) {
...
sub feature_set {
	...
	$self->{curr_feat}  = FAST::Bio::SeqFeature::Generic->new(
							    -primary => $stype,
							    -id      => $id,
							    );
	...
	if ( $cds ) {
	    ...
	    if ( @{$self->{curr_loc}} > 1 ) {
		my $loc = FAST::Bio::Location::Split->new( -splittype => 'JOIN' );
...
sub _add_feature_span {
    ...
    if ( $type eq 'exon' ) {
	my $sl = FAST::Bio::Location::Simple->new( -start  => $start,
                                             -end    => $end,
	...
	FAST::Bio::SeqFeature::Generic->new( -start   => $start,
				       -end     => $end,
				       -strand  => $self->{curr_strand},
				       -primary => $type );
    }
...
sub _add_CDS {
    ...
    if ( @{$self->{curr_loc}} > 1 ) {        
        $loc = FAST::Bio::Location::Split->new;
    ...
    for ( @exons ) {
        my $exon = FAST::Bio::Location::Simple->new;
    ...
    my $cds_loc;
    if ( @exons_to_add > 1 ) {
        $cds_loc = FAST::Bio::Location::Split->new( -splittype => 'JOIN'  );
        for ( @exons_to_add ) {
