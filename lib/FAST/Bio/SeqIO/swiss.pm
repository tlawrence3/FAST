#
# BioPerl module for FAST::Bio::SeqIO::swiss
#
...
This object can transform FAST::Bio::Seq objects to and from Swiss-Pprot flat
file databases.
...
A new FAST::Bio::AnnotationI class was created in order to store the
data in tag-value pairs.  This class (FAST::Bio::Annotation::TagTree)
is stored in the FAST::Bio::Annotation::Collection object and is
accessed like all other annotations. The tag name is 'gene_name'.

There is a single FAST::Bio::Annotation::TagTree per sequence record, which
corresponds to the original class that stored this data
(FAST::Bio::Annotation::StructuredValue).  Depending on how we progress
...
For more uses see FAST::Bio::Annotation::TagTree.
...
Also, for UniProt output we support using other FAST::Bio::AnnotationI, but in this
...
FAST::Bio::Annotation::SimpleValue or other FAST::Bio::AnnotationI classes.
...
through the FAST::Bio::SeqI interface method L<annotation()|annotation>.

The following are the names of the keys which are polled from a
L<FAST::Bio::Annotation::Collection> object.
 ...
 gene_name   - Should contain FAST::Bio::Annotation::SimpleValue object
...
sub next_seq {
    ...
    my $genename = "";
    my ($annotation, %params, @features) = ( FAST::Bio::Annotation::Collection->new());
    ...
    BEFORE_FEATURE_TABLE :
          while ( defined($_ = $self->_readline) ) {
              ...
              # Evidence level
                  ...
                  my $evidence = FAST::Bio::Annotation::SimpleValue->new
                      (-tagname   => 'evidence',
                       -value     => $line
                      );
              ...
              # Comments
                  ...
                  my $commobj = FAST::Bio::Annotation::Comment->new(-tagname => 'comment',
                                                              -text    => $comment);
              ...
              # new regexp from Andreas Kahari  bug #1584
                  ...
                  my $dblinkobj =  FAST::Bio::Annotation::DBLink->new
                      (-database    => $database,
...
sub write_seq {
    ...
    foreach my $seq ( @seqs ) {
        ...
        if ( $self->_id_generation_func ) {
            ...
            # if e.g. the FAST::Bio::DB::GenPept module is used as input.
        ...
        foreach my $gene ( my @genes = $seq->annotation->get_Annotations('gene_name') ) {
            # gene is a FAST::Bio::Annotation::TagTree;
            if ($gene->isa('FAST::Bio::Annotation::TagTree')) {
                my @genelines;
        ...
        # molecular weight
        my $mw = ${FAST::Bio::Tools::SeqStats->get_mol_wt($seq->primary_seq)}[0];
        # checksum
    ...
    while ( defined $_ ) {
        if ( /^[^R]/ || /^RN/ ) {
            if ( $rp ) {
                ...
                push @refs, FAST::Bio::Annotation::Reference->new
                    (-title   => $title,
                     -start   => $b1,
                     -end     => $b2,
                     -authors => $au,
...
sub _read_swissprot_Species {
    ...
    # if the organism belongs to taxid 32644 then no FAST::Bio::Species object.
...
sub _read_FTHelper_swissprot {
    ...
    my $out = FAST::Bio::SeqIO::FTHelper->new(-verbose => $self->verbose());
    $out->key($key);
    $out->loc($loc);
