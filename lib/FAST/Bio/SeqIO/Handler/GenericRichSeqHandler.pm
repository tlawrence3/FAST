#
# BioPerl module for FAST::Bio::SeqIO::Handler::GenericRichSeqHandler
#
...
from FAST::Bio::SeqIO::gbdriver, FAST::Bio::SeqIO::embldriver, and FAST::Bio::SeqIO::swissdriver
...
Though a FAST::Bio::Seq::SeqBuilder is employed for building sequence objects no
...
package FAST::Bio::SeqIO::Handler::GenericRichSeqHandler;
use strict;
use warnings;
    ...
    if( $dbsource =~ s/(UniProt(?:KB)|swissprot):\s+locus\s+(\S+)\,.+\n// ) {
        ...
        if( $dbsource =~ s/xrefs\s+\(non\-sequence\s+databases\):\s+
            ...
            for my $id ( split(/\,\s+/,$1) ) {
                ...
                $annotation->add_Annotation
                    ('dblink',FAST::Bio::Annotation::DBLink->new
                     (-primary_id => $id,
                      -database => $db,
                      -tagname => 'dblink'));
            }
...
sub _generic_dbsource {
    ...
    while ($data->{DATA} =~ m{([^\n]+)}og) { 
        ...
        if ( $dblink =~ m{([^\s;]+);\s*([^\s;]+);?\s*([^\s;]+)?}) {
            ...
            $link = FAST::Bio::Annotation::DBLink->new(-database    => $databse,
                                -primary_id  => $prim_id,
                                -optional_id => $sec_id);
        } else {
...
sub _generic_comment {
    ...
    $self->annotation_collection->add_Annotation('comment',
        FAST::Bio::Annotation::Comment->new( -text => $data->{DATA} ));
}
...
sub _generic_seqfeatures {
    ...
    my $sf = FAST::Bio::SeqFeature::Generic->direct_new();
    $sf->location($loc);
...
sub _generic_simplevalue {
    ...
    $self->annotation_collection->add_Annotation(
        FAST::Bio::Annotation::SimpleValue->new(-tagname => lc($data->{NAME}),
       -value => $data->{DATA})
        );
}
