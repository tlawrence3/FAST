#
# BioPerl module for FAST::Bio::SeqIO::bsml
#
 ...
 * Generate Seq objects with no sequence data but an assigned
   length. This appears to be an issue with FAST::Bio::Seq. It is possible
...
#       $self->sequence_factory(FAST::Bio::Seq::SeqFactory->new(-verbose => $self->verbose(), -type => 'FAST::Bio::Seq::RichSeq'));
#   }
# }
...
#>>>>   # This does not appear to work - length is apparently calculated
    ...
    my $species = FAST::Bio::Species->new();
    my @classification = ();
	...
	if( $name eq 'database-xref' ) {
	    ...
	    push @links, FAST::Bio::Annotation::DBLink->new(-primary_id => $link_id,
						      -database   => $link_db);
	}
	next if ($value ne "");
	...
	if ($name =~ /comment/) {
	    my $com = FAST::Bio::Annotation::Comment->new('-text' => $content);
	...
	my $simp = FAST::Bio::Annotation::SimpleValue->new( -value => $content);
...
#>>>>  This should be modified so that any IDREF associated with the
	...
	my $reference = FAST::Bio::Annotation::Reference->new( %refVals );
...
=head2 to_bsml
 ...
 Function: Generates an XML structure for one or more FAST::Bio::Seq objects.
 ...
 Args    : Argument array in form of -key => val. Recognized keys:
      ...
      -seq A FAST::Bio::Seq reference, or an array reference of many of them
...
#	push @{$seqDesc}, ["bio_object" , ref($bioSeq)];
	...
	# Check for FAST::Bio::Annotations on the * <Sequence> *.
	$self->_parse_annotation( -xml => $xml, -obj => $bioSeq,
				  -desc => $seqDesc, -refs => $seqRefs);

	# Incorporate species data
	if (ref($bioSeq->species) eq 'FAST::Bio::Species') {
	    # Need to peer into FAST::Bio::Species ...
...
#>>>>	# ...however, BSML does not have a hierarchy for Features
	...
	if (@features && !$args->{SKIPFEAT}{all}) {
		...
		# Check for FAST::Bio::Annotations on the * <Feature> *.
...
=head2 write_seq
 ...
 Function: Prints out an XML structure for one or more FAST::Bio::Seq objects.
           If $seqref is an array ref, the XML tree generated will include
...
=head2 _parse_location
 ...
 Args    : 0 The DOM::Document being modified
           ...
           2 Reference to the FAST::Bio::SeqFeature being analyzed
...
=head2 _parse_bsml_feature
 ...
 Returns : FAST::Bio::SeqFeature::Generic
...
sub _parse_bsml_feature {
    ...
    my $basegsf = FAST::Bio::SeqFeature::Generic->new();
       # score
       # frame
       # source_tag
...
#>>>>   # This is not working, I think the error is somewhere downstream
	...
	# location has a messed up start - FAST::Bio::SeqFeature::Generic ref
...
sub _parse_bsml_location {
    ...
    $gsf ||= FAST::Bio::SeqFeature::Generic->new();
...
=head2 _parseparams
 ...
 Function: Borrowed from FAST::Bio::Parse.pm, who borrowed it from CGI.pm
...
=head1 TESTING SCRIPT
 ...
 my $biostream = FAST::Bio::SeqIO->new( -file => "$readfile" );
 my $seq = $biostream->next_seq();

 # Write BSML from SeqObject
 my $bsmlout = FAST::Bio::SeqIO->new( -format => 'bsml',
 ...
 my $bsmlin = FAST::Bio::SeqIO->new( -file => "$tempspot/out.bsml",
				  -format => 'bsml');
 my $seq2 = $bsmlin->next_seq();

 # Write format back from Seq Object
 my $genout = FAST::Bio::SeqIO->new( -format => $readformat,
