# BioPerl module for FAST::Bio::SeqIO::entrezgene
#
...
=head1 SYNOPSIS
   ...
   my $seqio = FAST::Bio::SeqIO->new(-format => 'entrezgene',
                               -file => $file);
...
L<FAST::Bio::ASN1::EntrezGene>, a low level ASN parser built by Mingyi Liu 
...
with the gene, etc. Almost all of these are given as  L<FAST::Bio::AnnotationI> objects.
...
The second variable returned, C<$genestructure>, is a L<FAST::Bio::Cluster::SequenceFamily>
...
Example code which looks for ontology terms:
  ...
  my $eio = new FAST::Bio::SeqIO(-file => $file,
                           -format => 'entrezgene',
                           -service_record => 'yes');
...
This parser is based on FAST::Bio::ASN1::EntrezGene module.
...
package FAST::Bio::SeqIO::entrezgene;

use strict;
use FAST::Bio::ASN1::EntrezGene;
use FAST::Bio::Seq;
use FAST::Bio::Species;
use FAST::Bio::Annotation::SimpleValue;
use FAST::Bio::Annotation::DBLink;
use FAST::Bio::Annotation::Comment;
use FAST::Bio::SeqFeature::Generic;
use FAST::Bio::Annotation::Reference;
use FAST::Bio::SeqFeature::Gene::Exon;
use FAST::Bio::SeqFeature::Gene::Transcript;
use FAST::Bio::SeqFeature::Gene::GeneStructure;
use FAST::Bio::Cluster::SequenceFamily;

#use FAST::Bio::Ontology::Ontology; Relationships.... later
...
sub _initialize {
    ...
    $self->{_parser} = FAST::Bio::ASN1::EntrezGene->new( file => $param{-file} );

    #Instantiate the low level parser here (it is -file in Bioperl
    #-should tell M.)
    #$self->{_parser}->next_seq; #First empty record- bug in FAST::Bio::ASN::Parser
}
...
sub next_seq {
    ...
    $self->{_ann}        = FAST::Bio::Annotation::Collection->new();
    $self->{_currentann} = FAST::Bio::Annotation::Collection->new();
    my @alluncaptured;
	...
	#return new FAST::Bio::Seq (-id=>'Generif service record', -seq=>'')
	#    unless ($xval->{'track-info'}{geneid}== 283);
    return FAST::Bio::Seq->new( -id => 'Generif service record', -seq => '' )
      if (
    ...
    my $s1     = shift @lineage;
    my $s2     = shift @lineage;
    my $specie = FAST::Bio::Species->new(
    ...
    my $cluster = FAST::Bio::Cluster::SequenceFamily->new(
...
sub _process_refseq {
    ...
    foreach my $product (@products) {
        ...
        {    #Minimal data required
            my $cann = FAST::Bio::Annotation::Collection->new();
            $pid = $product->{accession};
            my $authority =
              exists( $product->{type} )
              ? $product->{type}
              : $product->{heading};
            my $nseq = FAST::Bio::Seq->new(
            ...
            if ( exists( $product->{products} ) && ( $product->{products} ) ) {
                ...
                push @uncaptured, $uncapt;
                my $simann = FAST::Bio::Annotation::SimpleValue->new(
                    -value   => $prodid,
                    -tagname => 'product'
                );
...
sub _add_to_ann {    #Highest level only
  ...
  #  $val=~s/\n//g;#Low level EG parser leaves this so we take care of them here
    ...
    my $simann =
      FAST::Bio::Annotation::SimpleValue->new( -value => $val, -tagname => $tag );
            ...
            foreach my $ccomm (@comm) {
                ...
                foreach my $loc (@loc) {
                        ...
                        && ( $loc->{text} =~ /Location/i ) )
                    {
                        ...
                        $nfeat = FAST::Bio::SeqFeature::Generic->new(
                            -start   => $l1,
                            -end     => $l2,
                            -strand  => $tags{strand},
                            -source  => $loc->{type},
                        ...
                        my $sfeatann = FAST::Bio::Annotation::Collection->new();
...
sub _process_src {
        ...
        && ( $src->{src}->{tag}->{str} ) )
    {
        ...
        foreach my $id (@sq) {
            ...
            my $simann = FAST::Bio::Annotation::DBLink->new(
                -database   => $db,
                -primary_id => $id,
...
sub _add_references {
    ...
    if ( ref($refs) eq 'ARRAY' ) {
        foreach my $ref (@$refs) {
            my $refan = FAST::Bio::Annotation::Reference->new(
                -database   => 'Pubmed',
                -primary_id => $ref
            );
...
sub _process_locus {
    ...
    if ( exists( $self->{_current}->{seqs}->{'int'}->{from} ) ) {
      ...
      #unless ($start) {print $locus->{seqs}->{'int'}->{from},"\n",$locus,"\n";}
        ...
        $strand =
          ...
          ;    #1 being default
        my $nfeat = FAST::Bio::SeqFeature::Generic->new(
            -start  => $start,
            -end    => $end,
            -strand => $strand,
    ...
    my $gstruct = FAST::Bio::SeqFeature::Gene::GeneStructure->new();
    foreach my $product (@products) {
        my ( $tr, $uncapt ) =
...
sub _process_products_coordinates {
    ...
    my $transcript = FAST::Bio::SeqFeature::Gene::Transcript->new(
        ...
        || ( exists( $coord->{'genomic-coords'}->{'packed-int'} ) ) )
    {
        ...
        foreach my $exon (@coords) {
            ...
            my $exonobj = FAST::Bio::SeqFeature::Gene::Exon->new(
                -start  => $exon->{from},
                -end    => $exon->{to},
                -strand => $strand
            );
...
sub _process_STS {
    ...
    foreach my $product (@comm) {
        my $sts = FAST::Bio::Ontology::Term->new(
        ...
        my $annterm = FAST::Bio::Annotation::OntologyTerm->new();
        $annterm->term($sts);
...
sub _process_go {
    ...
    foreach my $comp (@comm) {
        ...
        if ( ref( $comp->{comment} ) eq 'ARRAY' ) {
            ...
            my $annterm =
              FAST::Bio::Annotation::OntologyTerm->new( -tagname => 'Gene Ontology' );
            $annterm->term($term);
            $self->{_ann}->add_Annotation( 'OntologyTerm', $annterm );
        }
    }
...
sub _get_go_term {
    my $go       = shift;
    my $category = shift;
    my $refan    = FAST::Bio::Annotation::Reference->new(    #We expect one ref per GO
        -medline => $go->{refs}->{pmid},
        -title   => 'no title'
    );
    my $term = FAST::Bio::Ontology::Term->new(
...
sub _backcomp_ll {
    my $ann    = shift;
    my $newann = FAST::Bio::Annotation::Collection->new();
...
#        my $simann=FAST::Bio::Annotation::SimpleValue->new(-value=>$seq->desc,-tagname=>'comment');
#        $newann->add_Annotation($simann);
