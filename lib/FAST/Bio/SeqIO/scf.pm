...
Do not use this module directly. Use it via the FAST::Bio::SeqIO class, see
L<FAST::Bio::SeqIO> for more information.

=head1 DESCRIPTION

This object can transform .scf files to and from FAST::Bio::Seq::SequenceTrace
...
package FAST::Bio::SeqIO::scf;
use vars qw($DEFAULT_QUALITY);
use strict;
use FAST::Bio::Seq::SeqFactory;
use FAST::Bio::Seq::SequenceTrace;
...
sub next_seq {
    ...
    my $swq = FAST::Bio::Seq::Quality->new(
        -seq  =>   $creator->{'sequence'},
        -qual =>    $creator->{'qualities'},
        -id   =>    $name_comment
                                              );
    my $returner = FAST::Bio::Seq::SequenceTrace->new(
                                           -swq      =>   $swq,
...
sub _get_comments {
    my ($self,$buffer) = @_;
    my $comments = FAST::Bio::Annotation::Collection->new();
    ...
    if (@comments_split) {
        foreach (@comments_split) {
            ...
            if ($1 && $2) {
                my ($tagname, $text) = ($1, $2);
                my $comment_obj = FAST::Bio::Annotation::Comment->new(
                                     -text => $text,
                                     -tagname => $tagname);
...
sub write_seq {
    ...
    my ($swq) = $self->_rearrange([qw(TARGET)], %args);
     my $writer_fodder;
     if (ref($swq) =~ /FAST::Bio::Seq::SequenceTrace|FAST::Bio::Seq::Quality/) {
               if (ref($swq) eq "FAST::Bio::Seq::Quality") {
                         # this means that the object *has no trace data*
                         # we might as well synthesize some now, ok?
                    $swq = FAST::Bio::Seq::SequenceTrace->new(
                         -swq     =>   $swq
                    );
               }
     }
    else  {
    $self->throw("You must pass a FAST::Bio::Seq::Quality or a FAST::Bio::Seq::SequenceTrace object to write_seq as a parameter named \"target\"");
    }
...
sub _get_binary_traces {
    my ($self,$version,$ref,$sample_size) = @_;
          # ref _should_ be a FAST::Bio::Seq::SequenceTrace, but might be a
          # FAST::Bio::Seq::Quality
     my $returner;
     my $sequence = $ref->seq();
     ...
     if ( ref($ref) eq "FAST::Bio::Seq::Quality" ) {
          $traceobj = FAST::Bio::Seq::Quality->new(
               -target   =>   $ref
          );
...
# Returns : Nothing. Modifies the FAST::Bio::Seq::Quality that was passed as an
#      argument.
# Args    : A reference to a FAST::Bio::Seq::Quality
# Notes   : None.
#
#=cut
#
