...
(see L<FAST::Bio::SeqIO> for details).
...
FAST::Bio::Seq::Quality objects. See L<FAST::Bio::Seq::Quality> for details.
...
sub next_primary_qual {
    ...
    $sequence =~ s/\n+/ /g;
    if ($as_next_qual) {
        $qual = FAST::Bio::Seq::PrimaryQual->new(-qual       => $sequence,
                                           -id         => $id,
                                           -primary_id => $id,
                                           -display_id => $id,
                                           -desc       => $fulldesc
...
sub write_seq {
    ...
    if (!$source || ( !$source->isa('FAST::Bio::Seq::Quality') &&
                      !$source->isa('FAST::Bio::Seq::PrimaryQual')   )) {
        $self->throw("You must pass a FAST::Bio::Seq::Quality or a FAST::Bio::Seq::PrimaryQual".
