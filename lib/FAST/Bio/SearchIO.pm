#
# BioPerl module for FAST::Bio::SearchIO
#
...
=head1 SYNOPSIS
   ...
   my $searchio = FAST::Bio::SearchIO->new( -format => 'blastxml',
                                     -file   => 'blastout.xml' );
...
a L<FAST::Bio::Search::Result::ResultI> compliant object, which is an object that
...
package FAST::Bio::SearchIO;
use strict;
use warnings;

# Object preamble - inherits from FAST::Bio::Root::IO

use FAST::Bio::SearchIO::SearchResultEventBuilder;
...
@FAST::Bio::SearchIO::InternalParserError::ISA = qw(FAST::Bio::Root::Exception);

use Symbol;

use base qw(FAST::Bio::Root::IO FAST::Bio::Event::EventGeneratorI FAST::Bio::AnalysisParserI);
...
See L<FAST::Bio::Factory::ObjectFactoryI>, L<FAST::Bio::SearchIO::SearchWriterI>
...
# TODO: The below don't seem to be implemented (e.g. in FAST::Bio::SearchIO::blast)
#
...
#                    default = $FAST::Bio::SearchIO::blast::MAX_HSP_OVERLAP.
  ...
  if( $class =~ /FAST::Bio::SearchIO::(\S+)/ ) {
    ...
    if( defined $output_format ) {
        if( defined $param{'-writer'} ) {
            my $dummy = FAST::Bio::Root::Root->new();
        ...
        my $output_module = "FAST::Bio::SearchIO::Writer::".$output_format;
    ...
    return "FAST::Bio::SearchIO::${format}"->new(@args);
  }
}
...
sub _initialize {
    ...
    $self->attach_EventHandler(FAST::Bio::SearchIO::SearchResultEventBuilder->new(@args));
    $self->{'_reporttype'} = '';
...
See L<FAST::Bio::SearchIO::EventHandlerI>
...
sub attach_EventHandler{
    my ($self,$handler) = @_;
    return if( ! $handler );
    if( ! $handler->isa('FAST::Bio::SearchIO::EventHandlerI') ) {
        $self->warn("Ignoring request to attatch handler ".ref($handler). ' because it is not a FAST::Bio::SearchIO::EventHandlerI');
    }
...
sub writer {
    my ($self, $writer) = @_;
    if( ref($writer) and $writer->isa( 'FAST::Bio::SearchIO::SearchWriterI' )) {
        $self->{'_result_writer'} = $writer;
    }
    elsif( defined $writer ) {
        $self->throw("Can't set ResultWriter. Not a FAST::Bio::SearchIO::SearchWriterI: $writer");
    }
...
sub min_query_length {
    ...
    if (@_) {
        ...
        if ( $min_qlen =~ /\D/ or $min_qlen <= 0 ) {
            $self->throw(
                -class => 'FAST::Bio::Root::BadParameter',
...
sub _load_format_module {
  my ($self,$format) = @_;
  my $module = "FAST::Bio::SearchIO::" . $format;
  my $ok;
