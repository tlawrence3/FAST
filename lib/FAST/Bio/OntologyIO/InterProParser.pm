...
=head1 SYNOPSIS
    ...
    # don't use this module directly - use FAST::Bio::OntologyIO with instead
    my $ipp = FAST::Bio::OntologyIO->new( -format  => 'interpro',
...
package FAST::Bio::OntologyIO::InterProParser;
use strict;
#use Carp;
use XML::Parser::PerlSAX;
use FAST::Bio::Ontology::SimpleOntologyEngine;
use FAST::Bio::Ontology::TermFactory;
use FAST::Bio::OntologyIO::Handlers::InterProHandler;
...
=head2 new
 ...
 Example : $ipp = FAST::Bio::OntologyIO::InterProParser->new( 
                                  -file => 't/data/interpro.xml',
				  -ontology_engine => 'simple' )

 Returns : Object of class FAST::Bio::OntologyIO::InterProParser.
 Args    :
...
sub _initialize{
    ...
    my $ip_h = FAST::Bio::OntologyIO::Handlers::InterProHandler->new(
			                             -ontology_name => $name);

    if(! $eng) {
	$eng_type = 'simple' unless $eng_type;
	if(lc($eng_type) eq 'simple') {
	    $eng = FAST::Bio::Ontology::SimpleOntologyEngine->new();
	} else {
    ...
    $self->term_factory(FAST::Bio::Ontology::TermFactory->new(
				   -type => "FAST::Bio::Ontology::InterProTerm"))
...
=head2 next_ontology
 ...
 Returns : Returns the ontology as a FAST::Bio::Ontology::OntologyEngineI
           compliant object.
 Args    : 
