#
# BioPerl module for FAST::Bio::Tools::GFF
#
...
At the B<end> of parsing to get either a list or hashref of FAST::Bio::Seq
...
use base qw(FAST::Bio::Root::Root FAST::Bio::SeqAnalysisParserI FAST::Bio::Root::IO);
...
sub _parse_header{
   ...
   while(my $line = $self->_readline()){
	 ...
	 if($line =~ /^\#\#sequence-region\s+(\S+)\s+(\S+)\s+(\S+)\s*/){
	   ...
	   push @{ $self->{'segments'} }, FAST::Bio::LocatableSeq->new
	       (
		-id    => unescape($seqid),
		-start => $start,
		-end   => $end,
...
sub _parse_sequence {
    ...
    if ($line =~ /^\>(.*)/) {
        ...
        my $seqfactory = FAST::Bio::Seq::SeqFactory->new('FAST::Bio::Seq');
...
sub _gff2_string{
   my ($gff, $origfeat) = @_;
    my $feat;
   if ($origfeat->isa('FAST::Bio::SeqFeature::FeaturePair')){
   ...
   if( ! $feat->has_tag('Target') && # This is a bad hack IMHO
       ! $feat->has_tag('Group') &&
       $origfeat->isa('FAST::Bio::SeqFeature::FeaturePair') ) {
...
sub _gff3_string {
	...
	if ($tag eq 'Target' && ! $origfeat->isa('FAST::Bio::SeqFeature::FeaturePair')){  
	...
	for my $value ( $feat->each_tag_value($tag) ) {
	    if(  defined $value && length($value) ) { 
                ...
                if ( ref $value eq 'FAST::Bio::Annotation::Comment') {
                    $value = $value->text;
                }
	...
	# such as might be for FAST::Bio::DB::SeqFeature
...
# This accessor is used for accessing the FAST::Bio::Seq objects from a GFF3
# file; if the file you are using has no sequence data you can ignore
# this accessor

# This accessor returns a hash reference containing FAST::Bio::Seq objects,
# indexed by FAST::Bio::Seq->primary_id
