...
FAST::Bio::Seq::SeqWithQuality - Bioperl object packaging a sequence with its quality.
Deprecated class, use FAST::Bio::Seq::Quality instead!

=head1 SYNOPSIS

    use FAST::Bio::PrimarySeq;
    use FAST::Bio::Seq::PrimaryQual;
    use FAST::Bio::Seq::SeqWithQuality;

    # make from memory
    my $qual = FAST::Bio::Seq::SeqWithQuality->new
    ...
    my $seqobj = FAST::Bio::PrimarySeq->new
    ( -seq => 'atcgatcg',
      -id  => 'GeneFragment-12',
    ...
    my $swqobj = FAST::Bio::Seq::SeqWithQuality->new
    ( -seq  => $seqobj,
      -qual => $qualobj
    );
    # done!
...
package FAST::Bio::Seq::SeqWithQuality;
...
use base qw(FAST::Bio::Root::Root FAST::Bio::PrimarySeqI FAST::Bio::Seq::QualI);

=head2 new()

 Title   : new()
 Usage   : $qual = FAST::Bio::Seq::SeqWithQuality ->new
             ( -qual => '10 20 30 40 50 50 20 10',
               -seq => 'ATCGATCG',
               -id  => 'human_id',
               -accession_number => 'AL000012',
 ...
 Function: Returns a new FAST::Bio::Seq::SeqWithQual object from basic
           constructors.
 Returns : a new FAST::Bio::Seq::PrimaryQual object
 Args    : -qual can be a quality string (see FAST::Bio::Seq::PrimaryQual for more
            information on this) or a reference to a FAST::Bio::Seq::PrimaryQual
            object.
           -seq can be a sequence string (see FAST::Bio::PrimarySeq for more
            information on this) or a reference to a FAST::Bio::PrimaryQual object.
           -seq, -id, -accession_number, -primary_id, -desc, -id behave like
            this:
             1. if they are provided on construction of the
                 FAST::Bio::Seq::SeqWithQuality they will be set as the descriptors for
              ...
              a) $obj->set_common_descriptors() is used and both the -seq and
                 ...
                 FAST::Bio::Seq::SeqWithQual object.
...
sub new {
   ...
   if (!$seq) {
      ...
      unless ($self->{suppress_warnings} == 1) {
         $self->warn("You did not provide sequence information during the ".
           "construction of a FAST::Bio::Seq::SeqWithQuality object. Sequence ".
      ...
      if (!$alphabet) {
         $self->throw("If you want me to create a PrimarySeq object for your ".
           ...
           "the constructor requirements for a FAST::Bio::PrimarySeq object with no ".
           "sequence. Read the POD for it, luke.");
      }
      $self->{seq_ref} = FAST::Bio::PrimarySeq->new( -seq              =>  "",
                                               -accession_number =>  $acc,
                                               -primary_id       =>  $pid,
                                               -desc             =>  $desc,
                                               -display_id       =>  $id,
                                               -alphabet         =>  $alphabet );
   } elsif ($seq->isa('FAST::Bio::PrimarySeqI') || $seq->isa('FAST::Bio::SeqI')) {
      $self->{seq_ref} = $seq;
   } elsif (ref($seq)) {
      $self->throw("You passed a seq argument into a SeqWithQUality object and".
        " it was a reference ($seq) which did not inherit from FAST::Bio::SeqI or ".
        "FAST::Bio::PrimarySeqI. I don't know what to do with this!");
   } else {
      my $seqobj = FAST::Bio::PrimarySeq->new( -seq              => $seq,
                                         -accession_number => $acc,
                                         -primary_id       => $pid,
                                         -desc             => $desc,
                                         -display_id       => $id   );
   ...
   if (!defined($qual)) {
      $self->{qual_ref} = FAST::Bio::Seq::PrimaryQual->new( -qual             => "",
                                                      -accession_number => $acc,
                                                      -primary_id       => $pid,
                                                      -desc             => $desc,
                                                      -display_id       => $id, );
   } elsif (ref($qual) eq "FAST::Bio::Seq::PrimaryQual") {
      $self->{qual_ref} = $qual;
   } else {
      my $qualobj = FAST::Bio::Seq::PrimaryQual->new( -qual             => $qual,
                                                -accession_number => $acc,
                                                -primary_id       => $pid,
                                                -desc             => $desc,
                                                -display_id       => $id,
   ...
   $self->length();
   $self->deprecated("deprecated class - use FAST::Bio::Seq::Quality instead");
...
=head2 qual_obj
 ...
 Function: Get the PrimaryQual object that is imbedded in the
           ...
           SeqWithQuality object.
 Returns : A reference to a FAST::Bio::Seq::SeqWithQuality object.
...
sub qual_obj {
   my ($self,$value) = @_;
   if (defined($value)) {
      if (ref($value) eq "FAST::Bio::Seq::PrimaryQual") {
         $self->{qual_ref} = $value;
         ...
         $self->debug("You tried to change the PrimaryQual object within a ".
           ...
           " was not a FAST::Bio::Seq::PrimaryQual object. Thus your change failed. ".
           "Sorry.\n");	    
      }
   }
...
=head2 seq_obj
 ...
 Function: Get the PrimarySeq object that is imbedded in the
           ...
           SeqWithQuality object.
 Returns : A reference to a FAST::Bio::PrimarySeq object.
...
sub seq_obj {
   my ($self,$value) = @_;
   if( defined $value) {
      if (ref($value) eq "FAST::Bio::PrimarySeq") {
         ...
         $self->debug("You tried to change the PrimarySeq object within a ".
           "SeqWithQuality object but you passed a reference to an object that".
           " was not a FAST::Bio::PrimarySeq object. Thus your change failed. Sorry.\n");
      }
   }
   return $self->{seq_ref};
}
