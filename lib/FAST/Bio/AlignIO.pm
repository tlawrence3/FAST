#
# BioPerl module for FAST::Bio::AlignIO
#
#	based on the FAST::Bio::SeqIO module
...
=head1 SYNOPSIS
    ...
    $in  = FAST::Bio::AlignIO->new(-file   => $inputfilename ,
                             -format => 'fasta');
    $out = FAST::Bio::AlignIO->new(-file   => ">out.aln.pfam" ,
                             -format => 'pfam');
    ...
    $in  = FAST::Bio::AlignIO->newFh(-fh     => \*MYIN,
                               -format => 'fasta');
    open my $MYOUT, '>', 'testaln.pfam';
    $out = FAST::Bio::AlignIO->newFh(-fh     =>  $MYOUT,
                               -format => 'pfam');
...
L<FAST::Bio::AlignIO> is a handler module for the formats in the AlignIO set,
for example, L<FAST::Bio::AlignIO::fasta>. It is the officially sanctioned way 
of getting at the alignment objects. The resulting alignment is a
L<FAST::Bio::Align::AlignI>-compliant object. 
...
operations to read and write alignment objects:
    ...
    $stream = FAST::Bio::AlignIO->newFh(-format => 'Fasta');
...
L<FAST::Bio::AlignIO> is patterned on the L<FAST::Bio::SeqIO> module and shares
most of its features.  One significant difference is that
L<FAST::Bio::AlignIO> usually handles IO for only a single alignment at a time,
whereas L<FAST::Bio::SeqIO> handles IO for multiple sequences in a single stream.  
...
=head2 FAST::Bio::AlignIO-E<gt>new()

   $seqIO = FAST::Bio::AlignIO->new(-file => 'filename',   -format=>$format);
   $seqIO = FAST::Bio::AlignIO->new(-fh   => \*FILEHANDLE, -format=>$format);
   $seqIO = FAST::Bio::AlignIO->new(-format => $format);
   $seqIO = FAST::Bio::AlignIO->new(-fh => \*STDOUT, -format => $format);

The L<new> class method constructs a new L<FAST::Bio::AlignIO> object.  
...
example, to read from STDIN:
   ...
   $seqIO = FAST::Bio::AlignIO->new(-fh => \*STDIN);
...
Currently only those formats which were implemented in L<FAST::Bio::SimpleAlign>
have been incorporated into L<FAST::Bio::AlignIO>.  Specifically, C<mase>, C<stockholm>
and C<prodom> have only been implemented for input. See the specific module
(e.g. L<FAST::Bio::AlignIO::prodom>) for notes on supported versions.
...
=head2 FAST::Bio::AlignIO-E<gt>newFh()

   $fh = FAST::Bio::AlignIO->newFh(-fh   => \*FILEHANDLE, -format=>$format);
   # read from STDIN or use @ARGV:
   $fh = FAST::Bio::AlignIO->newFh(-format => $format);

This constructor behaves like L<new>, but returns a tied filehandle
rather than a L<FAST::Bio::AlignIO> object.  You can read sequences from this
...
evaluates as defined but false:
  ...
  my $clustal = FAST::Bio::AlignIO->new( -file   => "<prot.aln",
                                   -format => "clustalw" );
  my $msf = FAST::Bio::AlignIO->new(-file   => ">prot.msf",
                              -format => "msf",
...
use base qw(FAST::Bio::Root::Root FAST::Bio::Root::IO);

=head2 new

 Title   : new
 Usage   : $stream = FAST::Bio::AlignIO->new(-file => $filename,
                                       -format => 'Format')
 Function: Returns a new seqstream
 Returns : A FAST::Bio::AlignIO::Handler initialised with
...
sub new {
	...
	return "FAST::Bio::AlignIO::$format"->new(@args);
    }
}
...
=head2 newFh
 ...
 Title   : newFh
 Usage   : $fh = FAST::Bio::AlignIO->newFh(-file=>$filename,-format=>'Format')
 Function: does a new() followed by an fh()
 Example : $fh = FAST::Bio::AlignIO->newFh(-file=>$filename,-format=>'Format')
           $sequence = <$fh>;   # read a sequence object
           print $fh $sequence; # write a sequence object
 Returns : filehandle tied to the FAST::Bio::AlignIO::Fh class
 Args    :
...
sub _load_format_module {
  my ($self,$format) = @_;
  my $module = "FAST::Bio::AlignIO::" . $format;
  my $ok;
...
=head2 next_aln
 ...
 Returns : a FAST::Bio::Align::AlignI compliant object
 Args    :
...
sub next_aln {
   my ($self,$aln) = @_;
   $self->throw("Sorry, you cannot read from a generic FAST::Bio::AlignIO object.");
}
...
sub write_aln {
    my ($self,$aln) = @_;
    $self->throw("Sorry, you cannot write to a generic FAST::Bio::AlignIO object.");
}
...
=head2 alphabet
 ...
 Function: Get/Set alphabet for purpose of passing to FAST::Bio::LocatableSeq creation
