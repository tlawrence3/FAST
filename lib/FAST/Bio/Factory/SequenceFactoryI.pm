#
# BioPerl module for FAST::Bio::Factory::SequenceFactoryI
#
...
# get a FAST::Bio::Factory::SequenceFactoryI object like

    use FAST::Bio::Seq::SeqFactory;
    my $seqbuilder = FAST::Bio::Seq::SeqFactory->new('-type' => 'FAST::Bio::PrimarySeq');
...
reduces the amount of code that looks like
  ...
  if( $type  eq 'FAST::Bio::PrimarySeq' ) { ... } 
  elsif( $type eq 'FAST::Bio::Seq::RichSeq' ) { ... }
...
package FAST::Bio::Factory::SequenceFactoryI;

use strict;

use base qw(FAST::Bio::Factory::ObjectFactoryI);
...
=head2 create
 ...
 Usage   : my $seq = $seqbuilder->create(-seq => 'CAGT', 
					 -id => 'name');
 Function: Instantiates new FAST::Bio::PrimarySeqI (or one of its child classes)
