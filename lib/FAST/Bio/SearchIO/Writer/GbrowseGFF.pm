#-----------------------------------------------------------------
#
# BioPerl module FAST::Bio::SearchIO::Writer::GbrowseGFF.pm
#
...
package FAST::Bio::SearchIO::Writer::GbrowseGFF;
...
sub to_string {
    ...
    $format ||='3';
    my $gffio = FAST::Bio::Tools::GFF->new(-gff_version => $format); # try to set it
	...
	# in the GFF format since we are hard-coding the GFF output here :-(
		...
		$tags{'Gap'} = $cigar;
            ...
            my $formatter = FAST::Bio::Tools::GFF->new(-gff_version => $format);
