#
# BioPerl module for FAST::Bio::PullParserI
#
...
need a chunk. A chunk is just a FAST::Bio::Root::IO that contains all the raw data
...
See FAST::Bio::SearchIO::hmmer_pull for an example of implementing a parser using
PullParserI.
...
=head2 parent
 ...
 Returns : FAST::Bio::PullParserI
 Args    : none to get, OR FAST::Bio::PullParserI to set
...
=head2 chunk
 ...
 Args    : none to get, OR
           ...
           treat as the chunk, or FAST::Bio::Root::IO.
...
sub chunk {
    ...
    if (@_) {
        ...
        if (ref($thing) eq 'GLOB') {
            $self->{_chunk} = FAST::Bio::Root::IO->new(-fh => $thing);
        }
        elsif (ref(\$thing) eq 'SCALAR') {
            if ($thing !~ /\n/ && -e $thing) {
                $self->{_chunk} = FAST::Bio::Root::IO->new(-file => $thing);
            }
            else {
                unless ($FORCE_TEMP_FILE) {
                    # treat a string as a filehandle
                    open(my $fake_fh, "+<", \$thing); # requires perl 5.8
                    $self->{_chunk} = FAST::Bio::Root::IO->new(-fh => $fake_fh);
                }
                else {
                    my ($handle) = $self->{_chunk}->tempfile();
                    print $handle $thing;
                    $self->{_chunk} = FAST::Bio::Root::IO->new(-fh => $handle);
                }
            }
        }
        elsif ($thing->isa('FAST::Bio::Root::IO')) {
            $self->{_chunk} = $thing;
        }
        else {
