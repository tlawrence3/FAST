#
# BioPerl module for FAST::Bio::Seq
#
...
=head1 SYNOPSIS
    ...
    $seqio  = FAST::Bio::SeqIO->new( '-format' => 'embl' , -file => 'myfile.dat');
    $seqobj = $seqio->next_seq();

    # SeqIO can both read and write sequences; see FAST::Bio::SeqIO
    # for more information and examples

    # get from database
    $db = FAST::Bio::DB::GenBank->new();
    $seqobj = $db->get_Seq_by_acc('X78121');

    # make from strings in script
    $seqobj = FAST::Bio::Seq->new( -display_id => 'my_id',
    ...
    # features must implement FAST::Bio::SeqFeatureI interface
    ...
    # annotation objects are FAST::Bio::AnnotationCollectionI's
    $ann      = $seqobj->annotation(); # annotation object

    # references is one type of annotations to get. Also get
    # comment and dblink. Look at FAST::Bio::AnnotationCollection for
    # more information
    ...
    # all give back FAST::Bio::Seq objects themselves, though currently with no
    # features transfered
...
In Bioperl we have 3 main players that people are going to use frequently
  ...
  FAST::Bio::PrimarySeq  - just the sequence and its names, nothing else.
  FAST::Bio::SeqFeatureI - a feature on a sequence, potentially with a sequence
                     and a location and annotation.
  FAST::Bio::Seq         - A sequence and a collection of sequence features
...
map to file formats sensibly and for some bioinformaticians this might help
  ...
  FAST::Bio::PrimarySeq  - Fasta file of a sequence
  FAST::Bio::SeqFeatureI - A single entry in an EMBL/GenBank/DDBJ feature table
  FAST::Bio::Seq         - A single EMBL/GenBank/DDBJ entry
...
holding a reference to the sequence feature). See L<FAST::Bio::PrimarySeq> and
L<FAST::Bio::SeqFeatureI> for more information.
...
A simple and fundamental block of code:
  ...
  my $seqIOobj = FAST::Bio::SeqIO->new(-file=>"1.fa"); # create a SeqIO object
...
exons it finds as sequence features in Genbank Flat File format.
  ...
  $seqin = FAST::Bio::SeqIO->new( -format => 'EMBL' , -file => 'myfile.dat');
  $seqout= FAST::Bio::SeqIO->new( -format => 'Fasta', -file => '>output.fa');
...
using the -fh argument (see L<FAST::Bio::SeqIO> documentation for details).
...
The code above shows how a few FAST::Bio::Seq methods suffice to read, parse,
reformat and analyze sequences from a file. A full list of methods
available to FAST::Bio::Seq objects is shown below. Bear in mind that some of
these methods come from PrimarySeq objects, which are simpler
than Seq objects, stripped of features (see L<FAST::Bio::PrimarySeq> for
more information).
...
Please see L<FAST::Bio::Species> for more information on this object.
...
features.  See L<FAST::Bio::AnnotationCollectionI>,
L<FAST::Bio::Annotation::Collection>, L<FAST::Bio::Annotation::Reference>, and
L<FAST::Bio::Annotation::Comment> for details.
...
See L<FAST::Bio::PrimarySeq> for more details on PrimarySeq objects.
...
Please see L<FAST::Bio::SeqFeatureI> and L<FAST::Bio::SeqFeature::Generic>,
for more information on sequence features.

It is worth mentioning that one can also retrieve the start and end
positions of a feature using a FAST::Bio::LocationI object:

  $location = $feat->location # $location is a FAST::Bio::LocationI object
  $location->start;           # start position
  $location->end;             # end position

This is useful because one needs a FAST::Bio::Location::SplitLocationI object
in order to retrieve the coordinates inside the Genbank or EMBL join()
statements (e.g. "CDS    join(51..142,273..495,1346..1474)"):

  if ( $feat->location->isa('FAST::Bio::Location::SplitLocationI') &&
...
See L<FAST::Bio::LocationI> and L<FAST::Bio::Location::SplitLocationI> for more
information.
...
Note that this includes implementing FAST::Bio::PrimarySeqI.

=item FAST::Bio::IdentifiableI

=item FAST::Bio::DescribableI

=item FAST::Bio::AnnotatableI

=item FAST::Bio::FeatureHolderI
...
package FAST::Bio::Seq;
use strict;

use FAST::Bio::Annotation::Collection;
use FAST::Bio::PrimarySeq;

use base qw(FAST::Bio::Root::Root FAST::Bio::SeqI FAST::Bio::IdentifiableI FAST::Bio::DescribableI FAST::Bio::AnnotatableI FAST::Bio::FeatureHolderI FAST::Bio::AnnotationCollectionI);

=head2 new

 Title   : new
 Usage   : $seq = FAST::Bio::Seq->new( -seq => 'ATGGGGGTGGTGGTACCCT',
                                 -id  => 'human_id',
 ...
 Returns : a new FAST::Bio::Seq object
...
sub new {
    ...
    my $pseq = FAST::Bio::PrimarySeq->new(@args);
    ...
    my ($ann, $pid,$feat,$species) = &FAST::Bio::Root::RootI::_rearrange($self,[qw(ANNOTATION PRIMARY_ID FEATURES SPECIES)], @args);
	...
	if( defined $feat ) {
	    if( ref($feat) !~ /ARRAY/i ) {
		if( ref($feat) && $feat->isa('FAST::Bio::SeqFeatureI') ) {
		    $self->add_SeqFeature($feat);
		} else {
		    $self->warn("Must specify a valid FAST::Bio::SeqFeatureI or ArrayRef of FAST::Bio::SeqFeatureI's with the -features init parameter for ".ref($self));
		}
	    } else {
...
These methods implement the FAST::Bio::PrimarySeq interface by delegating
...
=head1 Methods from the FAST::Bio::PrimarySeqI interface
...
=head1 Methods for FAST::Bio::IdentifiableI compliance
...
=head1 Methods for FAST::Bio::DescribableI compliance
...
=head1 Methods for implementing FAST::Bio::AnnotatableI
...
=head1 Methods for delegating FAST::Bio::AnnotationCollectionI

=head2 get_Annotations()

 Usage   : my @annotations = $seq->get_Annotations('key')
 Function: Retrieves all the FAST::Bio::AnnotationI objects for a specific key
           for this object
...
=head2 add_Annotation()
 ...
 Usage   : $seq->add_Annotation('reference',$object);
           $seq->add_Annotation($object,'FAST::Bio::MyInterface::DiseaseI');
           $seq->add_Annotation($object);
           $seq->add_Annotation('disease',$object,'FAST::Bio::MyInterface::DiseaseI');
...
=head2 remove_Annotations()
 ...
 Function: Remove the annotations for the specified key from this sequence 
           object
 Returns : an list of FAST::Bio::AnnotationI compliant objects which were stored
...
=head1 Methods to implement FAST::Bio::FeatureHolderI
...
=head2 add_SeqFeature
 ...
 Function: Adds the given feature object (or each of an array of feature
           ...
           FAST::Bio::SeqFeatureI interface.
 Returns : 1 on success
 Args    : A FAST::Bio::SeqFeatureI implementing object, or an array of such objects.
...
=head2 remove_SeqFeatures
 ...
 Example :
 Returns : The array of FAST::Bio::SeqFeatureI objects removed from this seq.
 Args    : None
...
=head1 Methods provided in the FAST::Bio::PrimarySeqI interface


These methods are inherited from the PrimarySeq interface
and work as one expects, building new FAST::Bio::Seq objects
or other information as expected. See L<FAST::Bio::PrimarySeq>
for more information.
...
=head2 revcom
 ...
 Function: Produces a new FAST::Bio::Seq object which
 ...
 Returns : A new (fresh) FAST::Bio::Seq object
 Args    : None
...
These methods are specific to the FAST::Bio::Seq object, and not
found on the FAST::Bio::PrimarySeq object
...
sub primary_seq {
   ...
   if( defined $value) {
       if( ! ref $value || ! $value->isa('FAST::Bio::PrimarySeqI') ) {
	   $obj->throw("$value is not a FAST::Bio::PrimarySeq compliant object");
       }
...
=head2 species
 ...
 Returns : L<FAST::Bio::Species> object
 Args    : None or L<FAST::Bio::Species> object
