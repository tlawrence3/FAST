#
# BioPerl module for FAST::Bio::Root::IO
#
...
FAST::Bio::Root::IO - module providing several methods often needed when dealing with file IO

=head1 SYNOPSIS

    # utilize stream I/O in your module
    $self->{'io'} = FAST::Bio::Root::IO->new(-file => "myfile");
    ...
    $path = FAST::Bio::Root::IO->catfile($dir, $subdir, $filename);
...
sub _initialize_io {
    ...
    if($url){
        ...
        if($HAS_LWP) { #use LWP::UserAgent
            ...
            $input = $tempfile;
            $file  = $tempfile;
        } else { #use FAST::Bio::Root::HTTPget
            #$self->warn("no lwp");
    
            $fh = FAST::Bio::Root::HTTPget::getFH($url);
        }
        }
...
=head2 exists_exe
 ...
 Usage   : $exists = $obj->exists_exe('clustalw');
           $exists = FAST::Bio::Root::IO->exists_exe('clustalw')
           $exists = FAST::Bio::Root::IO::exists_exe('clustalw')
...
sub exists_exe {
    ...
    if($FILESPECLOADED) {
        foreach my $dir (File::Spec->path()) {
            my $f = FAST::Bio::Root::IO->catfile($dir, $exe);
            return $f if( -f $f && -x $f );
        }
    }
    return 0;
...
=head2 tempfile
 ...
 Args    : named parameters compatible with File::Temp: DIR (defaults to
           $FAST::Bio::Root::IO::TEMPDIR), TEMPLATE, SUFFIX.
...
=head2  tempdir
 ...
 Function: Creates and returns the name of a new temporary directory.
           ...
           temp directory. Use $FAST::Bio::Root::IO::TEMPDIR for that. Calling this
...
=head2 catfile
 ...
 Title   : catfile
 Usage   : $path = FAST::Bio::Root::IO->catfile(@dirs,$filename);
...
=head2 rmtree
 ...
 Title   : rmtree
 Usage   : FAST::Bio::Root::IO->rmtree($dirname );
