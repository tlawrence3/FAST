#
# BioPerl module for FAST::Bio::Tree::TreeFunctionsI
#
...
=head1 SYNOPSIS
  ...
  use FAST::Bio::TreeIO;
  my $in = FAST::Bio::TreeIO->new(-format => 'newick', -file => 'tree.tre');
...
package FAST::Bio::Tree::TreeFunctionsI;
use strict;

use base qw(FAST::Bio::Tree::TreeI);
...
=head2 remove_Node
 ...
 Args    : either FAST::Bio::Tree::NodeI or string of the node id
...
sub remove_Node {
   ...
   }  elsif( ! $input->isa('FAST::Bio::Tree::NodeI') ) {
       $self->warn("Did not provide either a valid FAST::Bio::Tree::NodeI object or id to remove_node");
       return 0;
   } else { 
       $node = $input;
   }
...
sub get_lineage_nodes {
    ...
    unless (ref $input) {
        ...
        $self->warn("Did not provide either a valid FAST::Bio::Tree::NodeI object or id to get_lineage_nodes");
        return;
    }
    else { 
        $node = $input;
    ...
    # when dealing with FAST::Bio::Taxon objects with databases, the root will always
...
=head2 splice
 ...
 Function: Remove all the nodes from a tree that correspond to the supplied
           ...
           -keep_*.
 Returns : n/a
 Args    : just a list of FAST::Bio::Tree::NodeI objects to remove, OR
...
sub splice {
    ...
    my @nodes_to_remove;
    if (ref($args[0])) {
        $self->throw("When supplying just a list of Nodes, they must be FAST::Bio::Tree::NodeI objects") unless $args[0]->isa('FAST::Bio::Tree::NodeI');
...
sub get_lca {
	...
	unless(ref($node) && $node->isa('FAST::Bio::Tree::NodeI')) {
...
=head2 merge_lineage
 ...
 Returns : n/a
 Args    : FAST::Bio::Tree::TreeI with only one leaf, OR
           FAST::Bio::Tree::NodeI which has an ancestor
...
sub merge_lineage {
    ...
    if ($thing->isa('FAST::Bio::Tree::TreeI')) {
    ...
    elsif ($thing->isa('FAST::Bio::Tree::NodeI')) {
...
=head2 is_binary
  ...
  Returns    : boolean
  Exceptions : 
  Args       : Internal node FAST::Bio::Tree::NodeI, optional
...
# FAST::Bio::TreeIO::newick::_write_tree_Helper
...
=head2 is_paraphyletic
 ...
 Args    : -nodes => Array of FAST::Bio::Tree::NodeI objects which are in the tree
           -outgroup => a FAST::Bio::Tree::NodeI to compare the nodes to
...
sub is_paraphyletic{
   ...
   my $tree = FAST::Bio::Tree::Tree->new(-root     => $clade_root,
				  -nodelete => 1);
...
=head2 reroot
 ...
 Args    : FAST::Bio::Tree::NodeI that is in the tree, but is not the current root

=cut

sub reroot {
    my ($self,$new_root) = @_;
    unless (defined $new_root && $new_root->isa("FAST::Bio::Tree::NodeI")) {
        $self->warn("Must provide a valid FAST::Bio::Tree::NodeI when rerooting");
        return 0;
    }
...
=head2 reroot_at_midpoint
 ...
 Returns : the new midpoint FAST::Bio::Tree::NodeIon success, 0 on failure
 Args    : non-root FAST::Bio::Tree::NodeI currently in $tree
...
sub reroot_at_midpoint {
    ...
    unless (defined $node && $node->isa("FAST::Bio::Tree::NodeI")) {
        $self->warn("Must provide a valid FAST::Bio::Tree::NodeI when rerooting");
        return 0;
    }
...
=head2 add_trait
  ...
  Description: Add traits to a FAST::Bio::Tree:Tree nodes
