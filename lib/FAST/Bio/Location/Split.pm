#
# BioPerl module for FAST::Bio::Location::Split
...
=head1 SYNOPSIS
    ...
    my $splitlocation = FAST::Bio::Location::Split->new();
    $splitlocation->add_sub_Location(FAST::Bio::Location::Simple->new(-start=>1,
							       -end=>30,
							       -strand=>1));
    $splitlocation->add_sub_Location(FAST::Bio::Location::Simple->new(-start=>50,
							       -end=>61,
							       -strand=>1));   
...
package FAST::Bio::Location::Split;
...
use base qw(FAST::Bio::Location::Atomic FAST::Bio::Location::SplitLocationI);
...
=head2 add_sub_Location
 ...
 Args    : list of FAST::Bio::LocationI implementing object(s) to add
...
sub add_sub_Location {
    ...
    my @locs;    
    foreach my $loc ( @args ) {
	if( !ref($loc) || ! $loc->isa('FAST::Bio::LocationI') ) {
	    $self->throw("Trying to add $loc as a sub Location but it doesn't implement FAST::Bio::LocationI!");
	    next;
	}	
