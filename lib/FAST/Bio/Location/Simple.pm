#
# BioPerl module for FAST::Bio::Location::Simple
...
=head1 SYNOPSIS
    ...
    my $location = FAST::Bio::Location::Simple->new(-start => 1, -end => 100,
                         -strand => 1 );
...
This is an implementation of FAST::Bio::LocationI to manage exact location
...
package FAST::Bio::Location::Simple;
use strict;

use base qw(FAST::Bio::Location::Atomic);
...
sub end {
	...
	if (!defined $self->{'_end'}) {
		if (!defined $self->{'_start'}) {
			$self->warn('Can not set FAST::Bio::Location::Simple::end() equal to start; start not set');
			return;
		}
...
sub trunc {
  ...
  if( $newstart < 1 || $newend > ($end-$start+1) ) {
    $out = FAST::Bio::Location::Simple->new();
