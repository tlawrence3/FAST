#
# BioPerl module for FAST::Bio::Annotation::Collection.pm
#
...
package FAST::Bio::Annotation::Collection;

use strict;

# Object preamble - inherits from FAST::Bio::Root::Root

use FAST::Bio::Annotation::TypeManager;
use FAST::Bio::Annotation::SimpleValue;


use base qw(FAST::Bio::Root::Root FAST::Bio::AnnotationCollectionI FAST::Bio::AnnotationI);
...
sub new{
   ...
   $self->{'_annotation'} = {};
   $self->_typemap(FAST::Bio::Annotation::TypeManager->new());

   return $self;
}


=head1 L<FAST::Bio::AnnotationCollectionI> implementing methods
  ...
  # if recursive search for keys recursively
    ...
    foreach my $key (@allkeys) {
      ...
      if ($keymatch) {
        ...
        foreach (@annotations) {
          if ($_->isa("FAST::Bio::AnnotationCollectionI")) {
            push (@anns, 
...
sub get_all_Annotations{
    ...
    return map {
	$_->isa("FAST::Bio::AnnotationCollectionI") ?
...
=head2 add_Annotation
 ...
 Usage   : $self->add_Annotation('reference',$object);
           $self->add_Annotation($object,'FAST::Bio::MyInterface::DiseaseI');
           $self->add_Annotation($object);
           $self->add_Annotation('disease',$object,'FAST::Bio::MyInterface::DiseaseI');
...
sub add_Annotation{
   ...
   if(ref($key) && $key->isa("FAST::Bio::AnnotationI") && (!ref($object))) {
   ...
   if( !ref $object ) {
       $self->throw("Must add an object. Use FAST::Bio::Annotation::{Comment,SimpleValue,OntologyTerm} for simple text additions");
   }

   if( !$object->isa("FAST::Bio::AnnotationI") ) {
...
=head1 FAST::Bio::AnnotationI methods implementations
...
sub description{
   ...
   if( defined $value ) {
       my $val = FAST::Bio::Annotation::SimpleValue->new();
       $val->value($value);
