#
# BioPerl module for FAST::Bio::Ontology::OntologyStore
#
...
of L<FAST::Bio::Ontology::OntologyI> instances from which an Ontology
...
package FAST::Bio::Ontology::OntologyStore;
use strict;

# Object preamble - inherits from FAST::Bio::Root::Root

use FAST::Bio::Ontology::DocumentRegistry;
...
sub get_ontology{
  ...
  if($name) {
    ...
    if(!$o){
      my $doc_registry = FAST::Bio::Ontology::DocumentRegistry->get_instance();
      my($url,$def,$fmt) = $doc_registry->documents($name);

      if(ref($url) eq 'ARRAY'){
        my $io = FAST::Bio::OntologyIO->new(-url      => $url,
                                      -defs_url => $def,
                                      -format   => $fmt,
                                     );
...
sub register_ontology {
  ...
  my $ret = 1;
  foreach my $ont (@args) {
    if(ref($ont) && $ont->isa('FAST::Bio::Ontology::OntologyI')){
	...
	if(! (ref($ont) && $ont->isa("FAST::Bio::Ontology::OntologyI"))) {
      $self->throw((ref($ont) ? ref($ont) : $ont)." does not implement ".
                   "FAST::Bio::Ontology::OntologyI or is not an object");
	}
...
sub remove_ontology{
	...
	$self->throw(ref($ont)." does not implement FAST::Bio::Ontology::OntologyI")
	    unless $ont && ref($ont) && $ont->isa("FAST::Bio::Ontology::OntologyI");
